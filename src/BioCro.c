	...
	)
{
	...
	int soilType, wsFun;
	double LeafN, LeafN_0, kLN,LeafN_1,dayofMaxLeafN;
	...
	/* Variables for Respiration Calculations. I am currently using hard values. I shall write a function that can takes these values as input. I have used optim function and data from Liu to get these values. */
...
/*		dbpS = sel_dbp_coef(REAL(DBPCOEFS), REAL(THERMALP), TT12c); */
		...
		dbpS = SUGARCANE_DBP_CUADRA(TT12c,TT0,TTseed,Tmaturity,Rd,Alm,Arm,Clstem,Ilstem,Cestem,Iestem,Clsuc,Ilsuc,Cesuc,Iesuc);
...
/*		if(LeafN > LeafN_0) LeafN = LeafN_0;      */
		...
		/* Here I am inserting function to evaluate cyclic variation in leaf N based on statisti     cal relationship provided in in Liu et al. (2001) Ecological Modeling 144(2001) pp 182-211 */

/* Lets Assume that minimum value of leaf N is 60% of LeafN_0 */

                LeafN_1=0.6*LeafN_0;
		if (lat >0 ) {
                dayofMaxLeafN = 235;
		} /* September Equinox for northern hemisphere*/
                
               if(lat <0) {
                dayofMaxLeafN = 80;
		}  /* March Equinox for Southern Hemisphere */
		LeafN=TOPLNseasonal(LeafN_0, LeafN_1,*(pt_doy+i),dayofMaxLeafN,365,lat);
		...
		alpha1 = (LeafN_0 - LeafN) * REAL(ALPHAB1)[0] + REAL(ALPHA)[0]; 
 		...
 		if(MinNitro < 0) MinNitro = 1e-3;
               ...
               if(Stem > 0 )
	               {
			       deepak_temp = (0.6*Stem)/(Stem+Leaf+Root);      
                               SugarStress = log(deepak_temp)/log(0.2);
			       if(SugarStress < 0 || SugarStress > 1) SugarStress = 1.0 ; 
                               SugarStress=1.0;
		       }
...
/******** Here I am calling function to calculate Growth Respiration. Currently Using a fraction of 0.242 is lost via growth respiration. This fraction is taken from DSSAT 4.5 manual for Sugarcane: parameter RespGcf *****/
	       ...
	       CanopyA=GrowthRespiration(CanopyA, gRespCoeff);   
                ...
                mResp=MRespiration(Leaf, Qleaf, mRespleaf, *(pt_temp+i), timestep);
		if(kLeaf > 0)
		{
			newLeaf = CanopyA * kLeaf * LeafWS*SugarStress;
			newLeaf=newLeaf-mResp;
...
/*			newLeaf = resp(newLeaf, mrc1, *(pt_temp+i));  */
			...
			{
		      	 *(sti+i) = newLeaf; 
                        }                      /* This populates the vector newLeafcol. It makes sense
		...
		*/
		...
		/* New Stem*/
                ...
                mResp=MRespiration(Stem, Qstem, mRespstem, *(pt_temp+i), timestep);
		if(kStem >= 0)
		{
			newStem = CanopyA * kStem ;
			newStem=newStem-mResp;
/*			newStem = resp(newStem, mrc1, *(pt_temp+i));   */
		...
		if(TT12c < SeneStem){
			...
			q++;
               	...
               	mResp=MRespiration(Root, Qroot, mResproot, *(pt_temp+i), timestep);
		if(kRoot > 0)
		{
			newRoot = CanopyA * kRoot ;
/*			newRoot = resp(newRoot, mrc2, *(pt_temp+i));*/
			newRoot=newRoot-mResp;
			if(newRoot>0)
			{
		          *(sti3+i) = newRoot;
			}
			...
			m++;
             ...
             mResp=MRespiration(Rhizome, Qroot, mResproot, *(pt_temp+i), timestep);
		
                if(kRhizome > 0)
		{
			newRhizome = CanopyA * kRhizome ;
/*			newRhizome = resp(newRhizome, mrc2, *(pt_temp+i));  */
		        newRhizome=newRhizome-mResp;	
			if(newRhizome>0)
			{
			 *(sti4+ri) = newRhizome;
			}
