                                                          ...
                                                          derivative_module_names(deriv_module_names)
{
    ...
    // Get lists of subsets of quantities
...
System::System(
    ...
    void (*print_fcn_ptr)(char const* format, ...)) : initial_state(init_state),
                                                      invariant_parameters(invariant_params),
                                                      varying_parameters(varying_params),
                                                      steady_state_module_names(ss_module_names),
                                                      derivative_module_names(deriv_module_names),
...

{
    ...
    try {
        ...
        auto ip_names = keys(invariant_params);
        auto vp_names = keys(varying_params);
        std::vector<std::vector<std::string>> quantity_name_vectors{istate_names, ip_names, vp_names, steady_state_output_names};
        ...
        std::vector<std::unordered_map<std::string, double>> quantities_vector{initial_state, invariant_params, at(varying_params, 0)};
    ...
    std::vector<std::string>& duplicate_variable_names)
{
    ...
    for (auto const& x : varying_parameters) {
        if (unique_variable_names.find(x.first) == unique_variable_names.end()) {
            ...
            duplicate_variable_names.push_back(std::string("Parameter '") + x.first + std::string("' from the varying parameters"));
    }
    ...
    for (auto const& x : initial_state) {
        if (unique_variable_names.find(x.first) == unique_variable_names.end()) {
            ...
            duplicate_variable_names.push_back(std::string("Parameter '") + x.first + std::string("' from the initial state"));
    }
    ...
    for (auto const& x : invariant_parameters) {
        if (unique_variable_names.find(x.first) == unique_variable_names.end()) {
            ...
            duplicate_variable_names.push_back(std::string("Parameter '") + x.first + std::string("' from the invariant parameters"));
    }
}
    ...
    std::set<std::string> const& unique_module_inputs) const
{
    ...
    if (found_unused_invariant_parameter)
        print_msg("You may want to consider removing them from the input list.\n\n");
    else
        print_msg("All invariant parameters were used as module inputs.\n\n");
