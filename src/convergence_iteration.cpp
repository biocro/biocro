...
state_map collatz_canopy::do_operation (state_map const& s) const
{
    ...
    for (unsigned int n = 0; n < 50; ++n) {
        ...
        //boundary_layer_conductance = leaf_boundary_layer_conductance(s.at("layer_wind_speed"), s.at("leafwidth"), s.at("temp"), leaf_temperature - s.at("temp"), conductance_in_m_per_s, layer_vapor_pressure);  // m / s
