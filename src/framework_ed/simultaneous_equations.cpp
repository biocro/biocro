...
#include "../framework/dynamical_system_helper_functions.h"
    ...
    mc_vector const& direct_mcs)
{
    ...
    num_problems += process_criterion<string_vector>(
        ...
        [=]() -> string_vector {
            return find_undefined_module_inputs(
                quantity_names,
                std::vector<mc_vector>{direct_mcs});
        },
        [](string_vector string_list) -> std::string {
            return create_message(
    ...
    string_vector unknown_quantities_from_modules = get_unknown_quantities(direct_mcs);
...
std::string analyze_simultaneous_equations_inputs(mc_vector const& direct_mcs)
{
    ...
    process_criterion<string_set>(
        message,
        [=]() -> string_set { return find_strictly_required_inputs(direct_mcs); },
    ...
    process_criterion<string_set>(
        message,
        [=]() -> string_set { return find_unique_module_outputs(direct_mcs); },
        [](string_set string_list) -> std::string { return create_message(
    ...
    process_criterion<string_vector>(
        message,
        [=]() -> string_vector { return get_unknown_quantities(direct_mcs); },
    ...
    mc_vector const& direct_mcs)
{
    ...
    bool valid = validate_simultaneous_equations_inputs(startup_message, known_quantities, unknown_quantities, direct_mcs);
    if (!valid) {
    ...
    direct_modules = get_module_vector(direct_mcs, quantities, &module_output_map);
    ...
    string_vector direct_output_names = string_set_to_string_vector(find_unique_module_outputs(direct_mcs));
    ...
    output_ptrs = get_const_pointers(direct_output_names, quantities);
    ...
    direct_ptrs = get_pointers(direct_output_names, module_output_map);
    ...
    direct_ptr_pairs = get_pointer_pairs(direct_output_names, quantities, module_output_map);
}
...
{
    string_set required_inputs = find_strictly_required_inputs(mcs);

    string_set module_outputs = find_unique_module_outputs(mcs);
