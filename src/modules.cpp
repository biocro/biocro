...
// This function generates a list of parameter names based on the
// number of layers.
std::vector<std::string> MultilayerModule::get_multilayer_param_names(
    int nlayers,
...
)
{
    ...
    for (size_t i = 0; i < param_names.size(); i++) {
        ...
        for (int j = 0; j < nlayers; j++) {
            param_name_stream << basename <<
                std::setfill('0') << std::setw(num_digits) << j;
...
const std::vector<double*> MultilayerModule::get_multilayer_op(
    std::unordered_map<std::string, double>* output_parameters,
    int nlayers, const std::string& name
) const
{
    std::vector<std::string> param_base_name_vector(1, name);
    std::vector<std::string> param_names =
        get_multilayer_param_names(nlayers, param_base_name_vector);
    ...
    for (int i = 0; i < nlayers; i++) {
        ...
        if (output_parameters->find(layer_name) != output_parameters->end()) {
            multilayer_op[i] = &((*output_parameters).at(layer_name));
        }
        else {
            throw std::logic_error(layer_name);
        }
    }
...
const std::vector<const double*> MultilayerModule::get_multilayer_ip(
...
) const
{
    ...
    for (int i = 0; i < nlayers; i++) {
        ...
        if (input_parameters->find(layer_name) != input_parameters->end()) {
            multilayer_ip[i] = &((*input_parameters).at(layer_name));
        }
        else {
            throw std::logic_error(layer_name);
        }
