...
class auto_solver : public system_solver
{
   ...
   private:
    ...
    std::string get_param_info() const override { return std::string("Solver used for adaptive systems:\n") + adaptive_solver->generate_info_report() + std::string("\nSolver used for other systems:\n") + other_solver->generate_info_report(); }
};
