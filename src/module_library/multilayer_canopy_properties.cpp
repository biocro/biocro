...
string_vector multilayer_canopy_properties::define_multiclass_multilayer_outputs()
{
    return {
        ...
        "incident_nir",        // micromol / (m^2 leaf) / s
...
void multilayer_canopy_properties::run() const
{
    ...
    // that the `PhotoCore::CanopyLight` constructor expects input expects PPFD
    // values, so we must convert photosynthetically active radiation (PAR) to
    // PPFD using the energy content of light in the PAR band
