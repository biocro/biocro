...
void ed_canac_leaf::do_operation() const
{
    ...
    const double Qp = collatz_PAR_flux * 1e6;                  // micromol / m^2 / s
    const double vmax = collatz_vmax * 1e6;                    // micromol / m^2 / s
    const double Rd = collatz_rd * 1e6;                        // micromol / m^2 / s
    const double Gs_min = conductance_stomatal_h2o_min * 1e3;  // mmol / m^2 / s
    const double Ca = mole_fraction_co2_atmosphere * 1e6;      // micromol / mol
    ...
    const photosynthesis_outputs first_c4photoC_output =
        c4photoC(
            Qp, temperature_air, rh, vmax, collatz_alpha, collatz_k,
            collatz_theta, collatz_beta, Rd, ball_berry_intercept,
            ball_berry_slope, Gs_min, StomataWS, Ca, atmospheric_pressure,
            water_stress_approach, collatz_rubisco_temperature_upper,
            collatz_rubisco_temperature_lower, gbw_guess);

    const double gsw_estimate = first_c4photoC_output.Gs;  // mmol / m^2 / s

    // Run EvapoTrans2 to get the leaf temperature
    const struct ET_Str evapotrans_output =
        EvapoTrans2(
            solar_energy_absorbed_leaf, solar_energy_absorbed_leaf,
            temperature_air, rh, windspeed, gsw_estimate, leafwidth,
    ...
    const photosynthesis_outputs second_c4photoC_output =
        c4photoC(
            ...
            evapotrans_output.boundary_layer_conductance);
