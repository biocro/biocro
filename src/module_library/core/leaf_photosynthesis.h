 ...
 *
 * `C3LeafResidual` collapses all three into a single 2D system with unknowns
...
/**
 * @brief All leaf-level outputs from a single forward evaluation of the C3
 * coupled photosynthesis / energy balance model.
 *
 * Returned by `C3LeafResidual::evaluate` after the Broyden solver has
 * converged on `(Ci, Tleaf)`.
 */
struct C3LeafOutputs {
    double An;  //!< Net CO2 assimilation rate (micromol / m^2 / s)
    double Gs;  //!< Stomatal conductance to H2O (mol / m^2 / s)
    double Vc;  //!< RuBP carboxylation rate = GrossAssim (micromol / m^2 / s)
    double RL;  //!< Non-photorespiratory CO2 release in the light (micromol / m^2 / s)
    double Rp;  //!< Photorespiration rate (micromol / m^2 / s)
};

struct C3LeafResidual {
    ...
    /**
     * @brief Evaluate the 2D residual at `x = {Ci, Tleaf}`.
     *
     * @param x `x[0]` = Ci (micromol/mol), `x[1]` = Tleaf (degrees C)
     * @return `{r_carbon (micromol/m²/s), r_energy (degrees C)}`
     */
    std::array<double, 2> operator()(std::array<double, 2> x) const
    {
    ...
    /**
     * @brief Extract all leaf-level outputs at a converged `(Ci, Tleaf)`.
     *
     * Call this once after the Broyden solver has converged to obtain `An`,
     * `Gs`, etc. without repeating any root-finding.
     */
    C3LeafOutputs evaluate(double Ci, double Tleaf) const
    {
        auto const s = _compute(Ci, Tleaf);
        double const Rp = (Ci > 0.0) ? s.Vc * s.Gstar / Ci : 0.0;  // micromol / m^2 / s
        return {s.An, s.Gs, s.Vc, s.RL, Rp};
    }

    /**
     * @brief Recommended initial guess: `{0.718 * Ca, ambient_temperature}`.
     *
     * `0.718 * Ca` matches the starting bracket midpoint used by the Dekker
     * solver inside `c3photoC`.
     */
    std::array<double, 2> initial_guess() const
    {
        return {Ca, ambient_temperature};
    }
   ...
   private:
    ...
    _State _compute(double Ci, double Tleaf) const
    {
        // --- 1. Temperature-adjusted kinetic parameters ----------------------
        c3_param_at_tleaf const c3p = c3_temperature_response(tr_param, Tleaf);
        ...
        double const Oi = o2 * o2_solubility(Tleaf);  // mmol / mol
        ...
        double const gbw_leaf = leaf_boundary_layer_conductance_campbell(
            ambient_temperature,
            Tleaf - ambient_temperature,
            leaf_width,
            wind_speed,
            atmospheric_pressure);                                                     // m / s
        double const gbw_mass = sequential_conductance(gbw_leaf, gbw_canopy);          // m / s
        double const gbw_mol = g_to_molecular(atmospheric_pressure, gbw_mass, Tleaf);  // mol / m^2 / s
        ...
        double const Gtc = sequential_conductance(gbw_mol / dr_boundary, Gs / dr_stomata);
        double const r_carbon = An - Gtc * (Ca - Ci);  // micromol / m^2 / s
        ...
        double const r_energy = check_leaf_temp(
            ...
            Tleaf, leaf_width, s, Gs, wind_speed);  // degrees C

        return {An, Gs, fvcb.Vc, RL, Gstar, r_carbon, r_energy};
    }
};
