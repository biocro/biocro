 ...
 *
 * @brief Uses the Penman-Monteith equation to determine leaf transpiration (see
 * eq. 14.4k from Thornley (1990), p. 408). Currently only intended for use by
 * Ed.
 *
 * See the "ed_nikolov_conductance_forced" module for a discussion about
 * converting molar conductances (with units of mol / m^2 / s) to energy
...
class ed_penman_monteith_transpiration : public direct_module
{
   public:
    ...
    static std::string get_name() { return "ed_penman_monteith_transpiration"; }
...
void ed_penman_monteith_transpiration::do_operation() const
{
    ...
    const double total_available_energy =
        solar_energy_absorbed_leaf - long_wave_energy_loss_leaf;  // W / m^2

    // Convert temperatures to Kelvin
    const double Tak =
        temperature_air + conversion_constants::celsius_to_kelvin;  // Kelvin

    // Calculate the volume of one mole of a gas at air temperature and pressure
    // using the ideal gas law
    const double volume_per_mol =
        physical_constants::ideal_gas_constant * Tak / atmospheric_pressure;  // m^3 / mol
    ...
    const double E_numerator =
        slope_water_vapor * total_available_energy +
        latent_heat_vaporization_of_water * psychrometric_parameter * ga * vapor_density_deficit;

    const double E_denomenator =
        latent_heat_vaporization_of_water *
        (slope_water_vapor + psychrometric_parameter * (1 + ga / gc));
