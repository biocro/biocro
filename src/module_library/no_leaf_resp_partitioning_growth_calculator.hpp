...
class no_leaf_resp_partitioning_growth_calculator : public SteadyModule {
    public:
        no_leaf_resp_partitioning_growth_calculator(const std::unordered_map<std::string, double>* input_parameters, std::unordered_map<std::string, double>* output_parameters) :
...
void no_leaf_resp_partitioning_growth_calculator::do_operation() const {
    ...
    if(canopy_assimilation_rate < 0) nonleaf_carbon_flux = 0.0;
    else nonleaf_carbon_flux = canopy_assimilation_rate;
    ...
    if(kLeaf > 0) {
        ...
        if(canopy_assimilation_rate < 0) newLeafcol = canopy_assimilation_rate;	// assimilation is negative here, so this removes leaf mass
        else newLeafcol = canopy_assimilation_rate * kLeaf;
