#ifndef LEAF_HEAT_BALANCE_MODULE_H
#define LEAF_HEAT_BALANCE_MODULE_H
...
{
/**
 *  @class leaf_heat_balance_module
 *
 *  @brief Uses `leaf_heat_balance()` to determine transpiration rate and leaf
 *  temperature.
 */
class leaf_heat_balance_module : public direct_module
{
   public:
    leaf_heat_balance_module(state_map const& input_quantities, state_map* output_quantities)
        : direct_module{},
    ...
    static std::string get_name() { return "leaf_heat_balance"; }
...
string_vector leaf_heat_balance_module::get_inputs()
{
    return {
...
string_vector leaf_heat_balance_module::get_outputs()
{
    return {
        "EPenman",           // mmol / m^2 / s
...
void leaf_heat_balance_module::do_operation() const
{
    ...
    leaf_heat_balance _leaf_heat_balance(
        absorbed_longwave,
        absorbed_shortwave,
