#ifndef THERMAL_TIME_SENESCENCE_LOGISTIC_SEED_H
#define THERMAL_TIME_SENESCENCE_LOGISTIC_SEED_H

#include "../modules.h"

class thermal_time_senescence_logistic_seed : public DerivModule {
	public:
		thermal_time_senescence_logistic_seed(const std::unordered_map<std::string, double>* input_parameters, std::unordered_map<std::string, double>* output_parameters) :
			// Define basic module properties by passing its name to its parent class
            DerivModule("thermal_time_senescence_logistic_seed"),
...
std::vector<std::string> thermal_time_senescence_logistic_seed::get_inputs() {
	return {
		"Leaf", "Stem", "Root", "Rhizome",
        "kSeneLeaf", "kSeneStem", "kSeneRoot", "kSeneRhizome",
		"kLeaf", "kStem", "kRoot", "kRhizome", "kGrain", "kSeed",
        "remobilization_fraction"
	};
}

std::vector<std::string> thermal_time_senescence_logistic_seed::get_outputs() {
	return {
		"Leaf", "LeafLitter","Stem", "StemLitter",
		"Root", "RootLitter", "Rhizome", "RhizomeLitter", "Grain", "Seed"
	};
}

void thermal_time_senescence_logistic_seed::do_operation() const {
    ...
    double kSeed = *kSeed_ip; // partitioning growth coefficient, dimensionless
    ...
    double dSeed = kSeed * senescence_leaf * remobilization_fraction; // change in seed biomass = new seed from remobilized leaf fraction, Mg/ha. Currently do not include seed senescence.
