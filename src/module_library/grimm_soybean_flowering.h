...
void grimm_soybean_flowering::do_operation() const
{
    ...
    // Calculate the growth rate based on the day of year, night length, temperature, and growth stage
    double rate_days_per_day;  // physiological days per calendar day
    if (doy_dbl < grimm_sowing_doy) {
        // The plant has not been sowed yet, so it can't grow
        rate_days_per_day = 0.0;
        ...
        if (temp < grimm_juvenile_T0) {
            ...
            rate_days_per_day = (temp - grimm_juvenile_T0) / (grimm_juvenile_T1 - grimm_juvenile_T0);
        } else if (temp < grimm_juvenile_T2) {
            // We are in the flat region at maximum growth
            rate_days_per_day = 1.0;
        } else if (temp < grimm_juvenile_T3) {
            // There is a linear decrease from 1 to 0 as temp changes from T2 to T3
            rate_days_per_day = 1.0 - (temp - grimm_juvenile_T2) / (grimm_juvenile_T3 - grimm_juvenile_T2);
        } else {
        ...
        double temperature_rate_factor;  // physiological days per calendar day
        if (temp < grimm_T_min) {
            // The temperature is too low for the plant to develop
            temperature_rate_factor = 0.0;
        } else if (temp < grimm_T_opt) {
            // There is a linear increase from 0 to 1 as temp changes from T_min to T_opt
            temperature_rate_factor = (temp - grimm_T_min) / (grimm_T_opt - grimm_T_min);
        } else {
            // The development rate is maxed out at 1
            temperature_rate_factor = 1.0;
        }
        double photoperiod_rate_factor;  // physiological days per calendar day
        if (night_length < grimm_N_min) {
            // The night is too short for the plant to develop
            photoperiod_rate_factor = 0.0;
        } else if (night_length < grimm_N_opt) {
            // There is a linear increase from 0 to 1 as night_length changes from N_min to N_opt
            photoperiod_rate_factor = (night_length - grimm_N_min) / (grimm_N_opt - grimm_N_min);
        } else {
            // The development rate is maxed out at 1
            photoperiod_rate_factor = 1.0;
        }
        rate_days_per_day = temperature_rate_factor * photoperiod_rate_factor;
    }

    // Decide if the plant is flowering
    double flowering_rate = 0.0;
    if (grimm_physiological_age > grimm_juvenile_pd_threshold + grimm_flowering_threshold) {
        // The plant is flowering
        flowering_rate = 1.0 / 24.0;
    }

    // Convert the rate to physiological days per hour
    const double rate_days_per_hour = rate_days_per_day / 24.0;

    // Update the output parameter list
    update(grimm_physiological_age_op, rate_days_per_hour);
    update(grimm_flowering_op, flowering_rate);
}
