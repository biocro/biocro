...
### The `styler` R package

We use an R package called `styler` to help ensure consistent style across our
R code. An R script is included within BioCro that uses `styler` to format the
code with appropriate settings.

To run the script, call the following command from an R session running in the
BioCro R package root directory:
