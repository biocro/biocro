...
C++ Standard Library, the and the Boost C++ Library.
    ...
    As for R package dependencies, the BioCro R package depends only upon
packages in the R standard library (stats and utils) for its basic installation
and functioning.  BioCro does use other, non-standard R packages for
building the vignettes (knitr) and for testing (testthat), but these
are not essential to a fully-functioning BioCro installation.
...
* We have a few additional rules specific to BioCro development:
  ...
  - The BioCro R package and repository follow semantic versioning of the form
    `major.minor.bug`. Hotfix branches should typically only increment the third
    (`bug`) number. Most release branches will increment the second (`minor`)
    number, and very rarely the first (`major`) number. Feature branches do not
    directly change the version number.

  - Branch names should be prefixed with the branch type: either `feature/`,
    `hotfix/`, or `release/`. Hotfix and release branches should be named with
    the new version number; for example, `hotfix/v3.0.3` or `release/v3.1.0`.
    Feature branch names should reflect their purpose, such as
    `feature/describe-branch-rules`.

  - `NEWS.md` should be updated whenever the package version changes.

* The following is a short description of this branching model:
  ...
  - The `develop` branch contains new BioCro features that are under development
    but not yet ready for a new release.

  - Hotfix branches are for relatively small changes that are nevertheless
    important enough to warrant immediate incorporation into `main`. Examples
    include bug fixes and crucial changes to GitHub workflows. A hotfix branch
    should be branched off `main`. When ready, it should first merged into
    `main` via an approved pull request. Then, a second pull request should be
    made to merge into `develop`. If there are no merge conflicts or test
    failures, this second request can be merged without any additional
    approval. When both merges are complete, the hotfix branch should be
    deleted.

  - Feature branches are for more complex changes that do not require immediate
    ...
    back into `develop` via an approved pull request. When the merge into
    `develop` is complete, the feature branch should be deleted.

  - When sufficient changes have accumulated in `develop` to justify a new
    version of the package, a release branch is used to move the changes from
    `develop` to `main`. Release branches should not include substantive
    changes; rather, a release branch is primarily used to increment the package
    version and to ensure that the changelog in `NEWS.md` is up-to-date. A
    release branch should be branched off `develop`. When it's ready, it should
    first merged into `main` via an approved pull request. Then, a second pull
    request should be made to merge into `develop`. If there are no merge
    conflicts or test failures, this second request can be merged without any
    additional approval. When both merges are complete, the release branch
    should be deleted.
