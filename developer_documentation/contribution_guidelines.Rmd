...
* Do not copy and paste code, changing only small parts. Choose a
  ...
  following R code, which calculates maximum values of several columns
  from a data frame called `biocro_result`:

  ```r
  max_leaf <- max(biocro_result$Leaf)
  max_stem <- max(biocro_result$Stem)
  max_root <- max(biocro_result$Root)
  ```

  The data and behavior can be separated by taking a functional
  programming approach:

  ```r
  col_for_max <- c('Leaf', 'Stem', 'Root')

  get_col_max <- function(colname) {
    max(biocro_result[[colname]])
  }

  max_vals <- sapply(col_for_max, get_col_max)
  ```
  ...
  place has a meaningful name: `get_col_max`. For a long list, this
  is succinct, and easier to understand and maintain.
