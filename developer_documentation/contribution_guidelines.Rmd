...
* The following is a short description of this branching model:
  ...
  - Hotfix branches are for relatively small changes that are nevertheless
    ...
    should be based off `main` and first merged into `main` via an approved pull
    request. Then, a second pull request should be made to merge into `develop`.
    If there are no merge conflicts or test failures, this request can be merged
    without any additional approval. When both merges are complete, the hotfix
  ...
  - Feature branches are for more complex changes that do not require immediate
    ...
    namespace. A feature branch should be based off `develop` and merged back
    into `develop`. When the merge into `develop` is complete, the feature
    branch should be deleted.
  ...
  - When sufficient changes have accumulated in `develop` to justify a new
    ...
    release branch should be based off `develop` and first merged into `main`
    via an approved pull request. Then, a second pull request should be made to
    merge into `develop`. If there are no merge conflicts or test failures, this
    request can be merged without any additional approval. When both merges are
    complete, the release branch should be deleted.
