...
* Beyond the basic description of git-flow, we have a few additional rules
  ...
  - `NEWS.md` should be updated whenever the package version changes, and
    whenever a feature branch is merged into `develop`. In the latter case, a
    description of the changes should be collected under an `UNRELEASED` section
    in `NEWS.md` so it is easier to keep track of new changes. If there is no
    `UNRELEASED` section in `NEWS.md` on `develop` when a feature branch is
    merged into `develop`, one should be added.
...
* The following is a short description of this branching model:
  ...
  - When sufficiently many changes have accumulated in `develop` to justify a
    ...
    version and to ensure an up-to-date changelog. A release branch should be
    branched off `develop`. When it's ready, it should first merged into `main`
    ...
    second request can be merged without any additional approval. When both
    merges are complete, the release branch should be deleted.
