...
* **Single ownership of each PR:** If multiple people are making changes to the
  ...
  - Pull request reviewers should only suggest changes, not implement them. If a
    reviewer wishes to make changes, it is better to make them in a new branch,
    and create a secondary PR to merge the changes into the original PR.
...
* **Submitted code should work:** It takes time and effort to produce a
  ...
  means that `R CMD check` does not produce any errors. While this does not
  guarantee that the new code achieves its goals, is fully documented, or
  follows the coding style guidelines, it is nevertheless a minimum requirement
  for any code to be incorporated into BioCro. The following is a thorough
  workflow that should ensure submitted code is working:

  1. Build the package and run the tests locally. The tests don't take much time
     to run so this is a good way to get fast feedback.

  2. When all the tests are passing, run `R CMD check` locally. This takes
     longer than just running the tests, but it will also check the examples and
     vignettes, which sometimes turns up more issues.
  ...
  3. When `R CMD check` is passing locally, make a PR, but don't assign any
     ...
     R. Sometimes an issue will appear that didn't occur locally on your own
...
* If there are `R CMD check` errors but you cannot figure out how to resolve
  them, a PR could be made to ask for help from other developers.
