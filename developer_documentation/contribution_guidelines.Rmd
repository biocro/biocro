...
* **Submitted code should work:** It takes time and effort to produce a
  ...
  1. Run the package tests locally. The tests don't take much time to run, so
     this is a good way to get fast feedback.

     Instructions for running the tests locally can be found in
     ["Running the _testthat_ Tests"](#running-testthat) (Chapter
     \@ref(running-testthat)).
...
* If there are `R CMD check` errors but you cannot figure out how to resolve
  them, a PR could be made to ask for help from other developers. Such a PR
  could be initially marked as a draft PR until the tests are passing.
