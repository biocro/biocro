...
R commands for submitting the package to the win-builder service and CRAN can be
found in the ["Submitting to CRAN" chapter](#cran-submission) below.

### Preparing a new version of BioCro on GitHub
...
4. If necessary, update the year in `LICENSE` and `LICENSE.md`

5. Change the `UNRELEASED` header in `NEWS.md` to the new version number, and
...
6. Run `R CMD check` to see any NOTEs that are reported; see the
...
7. Make two pull requests, requesting to merge the new branch into `develop` and
   `main`. In general, follow the guidelines in the
...
8. When the new version is on CRAN, merge the release branch into `develop` and
   `main`.
...
1. Make sure `R CMD check` does not produce warnings on any operating system or
   ...
   the package maintainer regardless of who performs this action. {#win-builder}
...
2. Submit the package, cross fingers, and wait. There are two ways to submit:
   ...
   1. Call `devtools::submit_cran()` from an R session running in the BioCro
      ...
      built package. {#devtools-submit-cran}
...
* The `src/module_library` directory is meant to be flat. Do not introduce new
  sub-directories without prior discussion and approval.
...
#### Be aware of module types and restrictions

* A differential module must return a time rate of change for each of its output
  variables (`dy / dt`). It should not return the absolute change in a variable
  over the next time step (`Delta y = y_next - y_current`) or the value of a
  variable at the next time step (`y_next`).

* Calculations within a module should be independent of the time step being used
  to solve the set of ordinary differential equations.

* If a module's code implementation is written in such a way that it will only
  return meaningful results when used with an Euler solver, it can be flagged as
  requiring an Euler solver. Most commonly, this occurs when a module stores an
  ordered sequence of previous state values under the assumption that the module
  is only evaluated once per time step. Such module implementations should be
