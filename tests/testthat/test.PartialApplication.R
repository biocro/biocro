...
test_that("functions generated by partial_run_biocro produce useful error messages", {
    skip(paste(
        "Skipping some tests of partial_run_biocro pending changes",
    ...
    expect_error(
        crop_func(list(weather$temp + temp_offset)),
        paste0(
            "The number of values passed to this function is not the number expected\\..*",
...
baseline_em_result <- evaluate_module(
    module,
    list(sowing_fractional_doy = 0, tbase = tbase, temp = temp, fractional_doy = 1)
)

# Now use partial application
ttc_func <- partial_evaluate_module(
    module,
    list(sowing_fractional_doy = 0, fractional_doy = 1),
    c("tbase", "temp")
)
