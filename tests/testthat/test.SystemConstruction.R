...
test_that("certain run_biocro inputs must not have empty elements", {
    expect_error(
        with(CROP, {
            run_biocro(
                within(initial_values, {
                    bad_initial_value <- numeric(0)
                }),
                within(parameters, {
                    bad_parameter <- numeric(0)
                }),
                WEATHER,
                direct_modules,
                differential_modules,
                within(ode_solver, {
                    bad_ode_solver_setting <- numeric(0)
                })
            )
        }),
        regexp = paste0(
...
test_that("certain run_biocro inputs must be numeric", {
    expect_error(
        with(CROP, {
            run_biocro(
                within(initial_values, {
                    bad_initial_value <- "terrible"
                }),
                within(parameters, {
                    bad_parameter <- "awful"
                }),
                within(WEATHER, {
                    bad_driver <- "offensive"
                }),
                direct_modules,
                differential_modules,
                within(ode_solver, {
                    bad_ode_solver_setting <- "heinous"
                })
            )
        }),
        regexp = paste0(
