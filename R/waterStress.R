...
##'
##'
wtrstr <- function(precipt, evapo, cws, soildepth, fieldc, wiltp, phi1 = 0.01, phi2 = 10, 
    wsFun = c("linear", "logistic", "exp", "none")) {
    wsFun <- match.arg(wsFun)
    ## precipitation data are entered in mm but need to convert to m
    precipM <- precipt * 0.001
    ## precipitation in meters can now be added to avaliable water to indicate the
    ## new avaliable water
    aw <- precipM + cws
    ...
    awha <- aw * 10000 * soildepth
    ## Since evaporation is entered in Mg H2O ha-1 This substraction can be made
    Newawha <- awha - evapo
    ## Need to convert back to new available water
    naw <- Newawha * 1e-04 * (1/soildepth)
...
wsRcoef <- function(aw, fieldc, wiltp, phi1, phi2, wsFun = c("linear", "logistic", 
    "exp", "none")) {
    wsFun <- match.arg(wsFun)
    if (wsFun == "linear") {
        slp <- 1/(fieldc - wiltp)
