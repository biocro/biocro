...
##' @param ...
##' @export
##' @return data.frame with NCEP met data
##' @author Fernando Miguez, Deepak Jaiswal
weachNEW <- function(X, lati, ts = 1, temp.units = c("Farenheit", "Celsius"), rh.units = c("percent", 
    ...
    for (i in 1:No.Of.Year) {
        ...
        if (LeapYearIndicator) {
            tempXOUT <- weach366(tempX, lati = lati, ts = ts, temp.units = temp.units, 
                rh.units = rh.units, ws.units = ws.units, pp.units = pp.units)
            output <- rbind(output, tempXOUT)
        }
        if (!LeapYearIndicator) {
            tempXOUT <- weach365(tempX, lati = lati, ts = ts, temp.units = temp.units, 
                rh.units = rh.units, ws.units = ws.units, pp.units = pp.units)
            output <- rbind(output, tempXOUT)
        }
    }
