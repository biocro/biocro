...
c3photo <- function(Qp, Tl, RH, vcmax = 100, jmax = 180, Rd = 1.1, Catm = 380, O2 = 210, b0 = 0.08, b1 = 5, theta = 0.7) {
    ...
    ## if(Catm < 20) warning('Stomatal conductance is not reliable for values of Catm lower than 20\n') if(Catm <
    ## 15) stop('Assimilation is not reliable for low (<15) Catm values')
    ...
    res <- .Call(c3photo_sym, as.double(Qp), as.double(Tl), as.double(RH), as.double(vcmax), as.double(jmax), as.double(Rd), 
        as.double(Catm), as.double(b0), as.double(b1), as.double(O2), as.double(theta))
    res
} 
