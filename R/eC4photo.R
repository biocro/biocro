...
## 
## Start of the functions related to the c4 photosynthesis model proposed by von Caemmerer This file will
## contain the functions in BioCro for and alternative photosynthesis model proposed by von Caemmerer This is
## the C4photo function based on von Caemmerer (2000)
...
##' }
##'
eC4photo <- function(Qp, airtemp, rh, ca = 380, oa = 210, vcmax = 60, vpmax = 120, vpr = 80, jmax = 400) {
    nqp = length(Qp)
    nat = length(airtemp)
    nrh = length(rh)
    if ((nqp != nat) || (nat != nrh)) {
    ...
    res <- .Call(eC4photo_sym, as.double(Qp), as.double(airtemp), as.double(rh), as.double(ca), as.double(oa), as.double(vcmax), 
        as.double(vpmax), as.double(vpr), as.double(jmax))
    res
}
...
##' }
##'
eCanA <- function(LAI, doy, hour, solarR, AirTemp, RH, WindS, Vcmax, Vpmax, Vpr, Jmax, Ca = 380, Oa = 210, StomataWS = 1) {
    
    
    inputs <- c(LAI, doy, hour, solarR, AirTemp, RH, WindS, WindS, Vcmax, Vpmax, Vpr, Jmax)
    if (length(inputs) != 12) 
        stop("The inputs should all be of length 1")
    
    
    res <- .Call(eCanA_sym, as.double(LAI), as.integer(doy), as.integer(hour), as.double(solarR), as.double(AirTemp), 
        as.double(RH), as.double(WindS), as.double(Ca), as.double(Oa), as.double(Vcmax), as.double(Vpmax), as.double(Vpr), 
        as.double(Jmax), as.double(StomataWS))
    res
}
...
##'
##'
MCMCEc4photo <- function(obsDat, niter = 30000, iCa = 380, iOa = 210, iVcmax = 60, iVpmax = 120, iVpr = 80, iJmax = 400, 
    thresh = 0.01, scale = 1) {
    assim <- obsDat[, 1]
    qp <- obsDat[, 2]
    temp <- obsDat[, 3]
    ...
    res <- .Call(McMCEc4photo, as.double(assim), as.double(qp), as.double(temp), as.double(rh), as.integer(niter), 
        as.double(iCa), as.double(iOa), as.double(iVcmax), as.double(iVpmax), as.double(iJmax), as.double(thresh), 
        as.double(scale))
...
plot.MCMCEc4photo <- function(x, x2 = NULL, x3 = NULL, type = c("trace", "density"), ...) {
    ...
    if (missing(x2) && missing(x3)) {
        if (type == "trace") {
            plot1 <- xyplot(x$resuMC[1, ] ~ 1:x$accept2, xlab = "Iterations", type = "l", ylab = expression(paste("Vcmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot2 <- xyplot(x$resuMC[2, ] ~ 1:x$accept2, xlab = "Iterations", type = "l", ylab = expression(paste("Vpmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot3 <- xyplot(x$resuMC[3, ] ~ 1:x$accept2, xlab = "Iterations", type = "l", ylab = expression(paste("Jmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
    ...
    } else ## This part of the code is to plot objects x and x2 Ploting the trace
    if (missing(x3)) {
        n1 <- x$accept2
        n2 <- x2$accept2
        ...
        if (type == "trace") {
            plot1 <- xyplot(tmpvec11 + tmpvec12 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Vcmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot2 <- xyplot(tmpvec21 + tmpvec22 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Vpmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot3 <- xyplot(tmpvec31 + tmpvec32 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Jmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            print(plot1, position = c(0, 0, 1, 0.333), more = TRUE)
            print(plot2, position = c(0, 0.333, 1, 0.666), more = TRUE)
            print(plot3, position = c(0, 0.666, 1, 1))
        ...
        if (type == "trace") {
            plot1 <- xyplot(tmpvec11 + tmpvec12 + tmpvec13 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Vmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot2 <- xyplot(tmpvec21 + tmpvec22 + tmpvec23 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Vpmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            plot3 <- xyplot(tmpvec31 + tmpvec32 + tmpvec33 ~ 1:minchainLength, xlab = "Iterations", type = "l", ylab = expression(paste("Jmax (", 
                mu, mol, " ", m^-2, " ", s^-1, ")")), ...)
            print(plot1, position = c(0, 0, 1, 0.333), more = TRUE)
            print(plot2, position = c(0, 0.333, 1, 0.666), more = TRUE)
            print(plot3, position = c(0, 0.666, 1, 1))
        } else if (type == "density") {
            plot1 <- densityplot(~tmpvec11 + tmpvec12 + tmpvec13, xlab = "Vmax", plot.points = FALSE, ...)
            plot2 <- densityplot(~tmpvec21 + tmpvec22 + tmpvec23, xlab = "Vpmax", plot.points = FALSE, ...)
            plot3 <- densityplot(~tmpvec31 + tmpvec32 + tmpvec33, xlab = "Jmax", plot.points = FALSE, ...)
            print(plot1, position = c(0, 0, 1, 0.333), more = TRUE)
            print(plot2, position = c(0, 0.333, 1, 0.666), more = TRUE)
            print(plot3, position = c(0, 0.666, 1, 1))
        }
