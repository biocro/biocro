get_growing_season_climate = function(climate) {
    doy_range = guess_growing_season_start_and_end_doy(climate)
    return(climate[with(climate, doy >= doy_range[1] & doy <= doy_range[2]), ])
}

guess_growing_season_start_and_end_doy = function(climate) {
    # Within the first half of the year, get the days on which the temperature was below 0.
    early_season_freezing_days = climate[with(climate, doy <= 183 & temp < 0), 'doy']
    ...
    late_season_freezing_days = climate[with(climate, doy > 183 & temp < 0), 'doy']
