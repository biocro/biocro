...
# below 0. The first day of the season is the last freezing day, or day 90 at
# the earliest.
#
# Within the second half of the year, get the days on which the temperature was
# below 0. The last day of the season is the first freezing day, or day 330 at
# the latest.
#
...
{
    early_season_freezing_days <-
        climate[with(climate, doy <= 183 & temp <= threshold_temperature), 'doy']

    day1 = max(c(early_season_freezing_days, 90))

    late_season_freezing_days <-
        climate[with(climate, doy > 183 & temp <= threshold_temperature), 'doy']

    dayn = min(c(late_season_freezing_days, 330))
