...
CanA <- function(lai, doy, hr, solar, temp, rh, windspeed, lat = 40, nlayers = 8, 
    kd = 0.1, StomataWS = 1, heightFactor = 3, photoControl = list(), lnControl = list(), 
    ...
    res <- .Call(CanA_sym, as.double(lai), as.integer(doy), as.integer(hr), as.double(solar), 
        as.double(temp), as.double(rh), as.double(windspeed), as.double(lat), as.integer(nlayers), 
        as.double(StomataWS), as.double(vmax), as.double(alpha), as.double(kparm), 
...
lnParms <- function(LeafN = 2, kpLN = 0.2, lnb0 = -5, lnb1 = 18, lnFun = c("none", 
    "linear")) {
