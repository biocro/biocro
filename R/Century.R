...
Century <- function(LeafL, StemL, RootL, RhizL, smoist, stemp, precip, leachWater, 
    ...
    ## Tm is the effect of soil texture on active SOM turnover The flow constants
    ## are taken from the paper Parton et al. 1987 SSSJ 51:1173 and Parton et al.
    ## 1993 Global Biogeochemistry pg 785
    Ks <- centuryP$Ks
    if (timestep == "week") {
    ...
    ## SC1.Ln = C1.5.Leaf.Ln$SC + C1.5.Stem.Ln$SC Collect respiration
    ...
    C3.6.Root = flow(SC3.Root, CN.active, Abiot, FmLc.Root$Lc, Tm, respC3.6, 2, Ks, 
        verbose)
    C4.6.Root = flow(SC4.Root, CN.active, Abiot, FmLc.Root$Lc, Tm, respC3.6, 6, Ks, 
        verbose)
    ## Rhizome
    SC3.Rhiz = SC3.Rhiz + 0.7 * SC3
    SC4.Rhiz = SC4.Rhiz + 0.7 * SC4
    C3.6.Rhiz = flow(SC3.Rhiz, CN.active, Abiot, FmLc.Rhiz$Lc, Tm, respC4.6, 2, Ks, 
        verbose)
    C4.6.Rhiz = flow(SC4.Rhiz, CN.active, Abiot, FmLc.Rhiz$Lc, Tm, respC4.6, 6, Ks, 
        verbose)
