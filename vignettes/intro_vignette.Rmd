...
```{r}
lowphi <- soilParms(FieldC = 0.37, WiltP = 0.2, phi2 = 1)
highphi <- soilParms(FieldC = 0.37, WiltP = 0.2, phi2 = 4)

lowphirun <- BioGro(cmi04, soilControl = lowphi)
highphirun <- BioGro(cmi04, soilControl = highphi)

library(lattice)
xyplot(lowphirun$SoilWatCont + highphirun$SoilWatCont ~ lowphirun$DayofYear, type = 'l')
```
