...
result = Gro(sorghum_initial_values, sorghum_parameters,
...
\item Define parameters for the \Rcode{Gro_solver} function.  Assuming
  ...
  \begin{Rlisting}[4]
    Gro_solver(initial_values, invariant_parameters, drivers,
  ...
  $\p_2$, \dots, $\p_k$, drivers $\d_1$, $\d_2$, \dots,
  $\d_m$, and initial values variables $\iv_1$, $\iv_2$, \dots,
...
\item Define parameters for the \Rcode{Gro_solver} function.  Again,
  ...
  As for \Rcode{initial_values}, \Rcode{invariant_parameters}, and
