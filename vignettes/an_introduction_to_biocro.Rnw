...
\lstnewenvironment{sh_listing}[1][2]
{\lstset{
    style=sh_style,
    gobble=#1
}}
...
\subsection[Adding new models]{Adding new models\footnote{Up until
  now, we have used the term \emph{model} to refer to the system as a
  whole, and the dynamics of how it changes over time.  Here we are
  using it in a more specialized sense: we \emph{model} the
  relationship between attributes of a state---how the values of some
  select group of attributes of the state determine the values or the
  rate of change of the values of other attributes of the state.  A
  model of the dynamics of soil evaporation, for instance, is a
...
\item Decide on a name for the module.  While in theory, the name
  ...
  \footnote{
    ...
    A valid C++ identifier must consist only of letters,
...
\item Begin a declaration of a class for the module, deriving it from
  ...
  readers of your code.}  Include a Doxygen-style comment (that is,
  starting with \code{/**}) describing the module and its purpose
  immediately above the class declaration.  The first sentence should
  be a brief summary of the module.  This is followed by additional
  details or a fuller description.  This is an appropriate place to
  include citations to sources for equations used by the module in
  ...
  appropriate.\footnote{The code that actually \emph{uses} the said
  equations will most likely be in the body of the \code{do_operation}
  function (or in some subsidiary function called by it).  But
...
\item Inside the body of the class declaration, declare private data
  ...
  type \code{const double&};
  by convention, the identifier used for the reference should be
  called \placeholder{parameter name}, that is, it should match the
  name of the corresponding parameter.%
  \footnote{Most existing modules use pointers rather than references
  for the input parameter values.  But there is no reason not to use a
  reference, and it saves one from having to dereference the pointer
  when the value is used.}\textsuperscript{,}%
...
\item Declare a public constructor with signature \placeholder{class
  ...
  parameter variable needs to be initialized with an initializer of the
  form
  \begin{C++listing}[4]
...
\newline\code{src/module_library/solar_zenith_angle_in_degrees.h}.
...
it won't corrupt any existing, complete documentation).  Thus, we set
...
that sets up an runs a complete system; and so here, by way of some
...
\item Define parameters for the \Rcode{Gro_solver} function.  Assuming
  ...
  values and may be omitted.)\footnote{
    ...
    writing.  In this case, values for both \Rcode{solver} and
...
\item Write one or more tests.
  ...
  \item Sample values that produce recognizable results.
    ...
    we can write our test as follows:\footnote{
...
\subsection{Writing a unit test for a derivative module using Gro\_solver}
...
variables.\footnote{
  In the case of a deriv module where some of the inputs correspond to
  outputs of steady-state modules, we have two choices with regard to
  testing: (1) include, as part of the test, steady-state modules that
  produce these required inputs; (2) artificially include values for
  these inputs as part of the varying parameters.  The latter option
  is the simplest and has the advantage of narrowing the scope of the
  test to the module we are most interested in testing.  But there may
  be cases where we want to test how two or more modules work in
