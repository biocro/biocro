...
that the tests called the \Rcode{Gro_solver} function.  This entails
testing a module by setting up a complete system and running a full
simulation.

Using the \Rcode{test_module} function instead is a much more
convenient way to write tests: It can be used to test the basic
functioning of a module while avoiding all of the many steps needed to
set up a full system for running a simulation.

There may be cases, however, where we may still want the flexibility
offered by being able to specify and run a full simulation.
Therefore, it was felt useful to present the steps required in this
appendix.  Thus, we re-present the material from section
\ref{sec:writing_tests}, showing how to write tests using
\Rcode{Gro_solver} instead of \Rcode{test_module}.

\subsection{Writing a unit test for a steady-state module}
...
\subsection{Writing a unit test for a derivative module}
