...
\item Define parameters for the \Rcode{Gro_solver} function.  Assuming
  ...
  \begin{Rlisting}[4]
    Gro_solver(initial_state, invariant_parameters, drivers,
  ...
  $\ip_2$, \dots, $\ip_k$, drivers $\d_1$, $\d_2$, \dots,
  $\d_m$, and initial values variables $\isv_1$, $\isv_2$, \dots,
  ...
  \begin{Rlisting}[4]
    ...
    initial_values <- list(isv_1 = ___, isv_2 = ___, ..., isv_n = ___)
  \end{Rlisting}
