...
\item Implement the \code{do_operation()} function.  This is the heart
  ...
  \begin{example}[4]
    void heating_degree_days::do_operation() const {
        double temperature_deficit { base_temperature > temp_ip ?
                                     (base_temperature - temp_ip) : 0};
...
documentation in various configurations.  While in the process of
\emph{writing} documentation, however, it is convenient to be able to
quickly genererate the HTML- or PDF-style documents for the portion of
the documentation you are working on.  One way to do this is as
follows.
...
command, which generates \emph{all} of the HTML documentation for the
C++ source code, and overridden the default set of source files and
the default documentation output directory for Make target \emph{view}
so that we only spend time documenting the file of interest and so
that the documentation is output to a different place so that it won't
corrupt any existing, complete documentation.\footnote{The name
...
\item Write one or more tests.
  ...
  \item Sample values that produce recognizable results.
    ...
    from our module with a simple test.  We can write our test as follows:\footnotemark
