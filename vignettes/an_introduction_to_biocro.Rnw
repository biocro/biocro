...
This document uses some outdated terminology and needs to be updated to reflect
some of the recent changes to BioCro.
...
\begin{tabular}{| r | l |}
    ...
    \code{differential\_module\_names} & $\h$ \\
    \hline
\end{tabular}
...
argument.\footnote{The \code{Gro} function also uses an R \code{list}
  ...
  ``direct'' modules, the other names of ``differential'' modules.
  These are then passed to \code{Gro\_solver}.}

Lists of parameters and modules are provided for soybean, miscanthus,
...
str(head(willow$initial_values)) # head truncates the list to six items
@
...
<<weather2005>>=
head(get_growing_season_climate(weather$'2005'))
@
...
result <- with(miscanthus_x_giganteus, {run_biocro(
  initial_values,
  parameters,
...
xyplot(Stem + Leaf + Root ~ TTc, data=result)  # `xyplot` requires the lattice package.
...
for writing direct modules and for writing differential modules.
...
\item Add \emph{include} directives to include the module base class
  declarations from \code{module.h} and the type aliases from
  \code{state_map.h}.
  \begin{example}[4]
    #include "../module.h"
    #include "../state_map.h"
...
\item Begin a declaration of a class for the module, deriving it from
  \code{direct\_module}.  The identifier used for the class name should match
...
\item Declare a public constructor with signature
  \begin{C++listing}[4]
    |\placeholder{class name}|(const state_map* input_quantities, state_map* output_quantities)
  \end{C++listing}
  ...
  \begin{C++listing}[4]
    |\placeholder{parameter name}|{get_input(input_quantities, |\stringplaceholder{parameter name}|)}
  \end{C++listing}
  (for input parameters), or
  \begin{C++listing}[4]
    |\placeholder{parameter name}|_op{get_op(output_quantities, |\stringplaceholder{parameter name}|)}
  \end{C++listing}
  ...
  \footnote{In existing modules, where pointers rather than references
      ...
      \lstinline!get_ip(input_quantities,%
...
\code{module_library/module_library.cpp} and requires two
steps:
...
\item Add an entry to
  \code{module_library::module_wrapper_creators}.  This
...
\subsubsection{Example: Steps for creating a differential module.}
...
\item Begin a declaration of a class for the module, deriving it from
  \code{differential\_module}.  The identifier used for the class name should
  match the module name you decided upon in step 1.
  \begin{example}[4]
...
\item Declare a public constructor with signature
  ...
  \end{C++listing}
  The constructor needs to initialize the base class
  \code{differential_module} with the name for the module, and each parameter
  variable needs to be initialized with an initializer of the form
  \begin{C++listing}[4]
    |\placeholder{parameter name}|{get_input(input_quantities, |\stringplaceholder{parameter name}|)}
  \end{C++listing}
...
\item Implement the \code{do_operation()} function.  This is the heart
  ...
  numerical value calculated in the body of the function prior to the
  update statement, and it will generally be calculated from the
  values of the input parameters.  Since this is a \emph{differential}
...
\code{module_library/module_library.cpp}:
  ...
  \item Sample values that produce recognizable results.
    ...
    \begin{Rexample}[6]
      test_that("When the cosine is 1/2, the angle is 60 degrees.", {
        input_quantities <- list(cosine_zenith_angle = 0.5)
...
\subsubsection{Writing unit tests for a differential module}
\label{sec:write_deriv_unit_test}
  ...
  differential module, the output values represent the rate of change of
...
\item Define parameters for the \Rcode{run_biocro} function.  Assuming
  ...
  \begin{Rlisting}[4]
    run_biocro(initial_values, parameters, drivers,
               direct_modules, differential_modules, solver, verbose)
  \end{Rlisting}
...
\subsection{Writing unit tests for a differential module using Gro\_solver}
...
variables.\footnote{
  In the case of a differential module where some of the inputs
...
\item Define parameters for the \Rcode{run_biocro} function.  Again,
  ...
  \begin{Rlisting}[4]
    direct_modules <- c()
    differential_modules <- c(|\stringplaceholder{name of module being tested}|)
  \end{Rlisting}
