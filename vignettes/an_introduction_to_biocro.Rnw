...
The R function \Rcode{biocro_simulation()} accepts five parameters that
...
result = biocro_simulation(sorghum_initial_values, sorghum_parameters,
             get_growing_season_climate(weather05),
             sorghum_steady_state_modules, sorghum_derivative_modules,
             sorghum_numerical_integrator)
...
that the tests called the \Rcode{biocro_simulation} function.  This method of
...
\item Define parameters for the \Rcode{biocro_simulation} function.  Assuming
  we call \Rcode{biocro_simulation} as
  \begin{Rlisting}[4]
    biocro_simulation(initial_values, parameters, drivers,
...
Here, we show how to test the heating degree days module using \Rcode{biocro_simulation}.
...
Again, we need an extra step for setting up some \Rcode{biocro_simulation} parameters:

\begin{enumerate}
\setcounter{enumi}{2}

\item Define parameters for the \Rcode{biocro_simulation} function.  Again,
  we will be assuming we call \Rcode{biocro_simulation} with
  \begin{Rlisting}[4]
    biocro_simulation(initial_values, parameters, drivers,
               steady_state_modules, derivative_modules, solver, verbose)
  \end{Rlisting}
...
\lstinputlisting[style=Rstyle]{an_introduction_to_biocro/test-solar_zenith_angle_in_degrees_with_biocro_simulation.R}

\subsection{Full listing of the unit-test file for heating-degree-days---Gro\_solver version} \label{hddgs test file}

\lstinputlisting[style=Rstyle]{an_introduction_to_biocro/test-heating_degree_days_with_biocro_simulation.R}
