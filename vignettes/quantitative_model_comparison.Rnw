...
particular plays a key role in BioCro's validated soybean model, and it can be
...
behavior of these two models, both on their own and in the context of a larger
...
is contained in the Sweave file used to create it. This is to place emphasis on
the BioCro functionality related to analyzing modules, since this part of the
code will be of more general use. For other analyses, the required weather data
will likely be different, and each user may have their own preferences regarding
plotting that differ from the strategies used here.
...
\item \texttt{catm}: A list of the atmospheric [\cotwo] levels for 1980 - 2020.
...
\item \texttt{weather}: A list of weather data where each named element
corresponds to the growing season of one particular year. Essentially, this
list contains growing seasons of weather data from Champaign, Illinois. Full
...
colors, combine panels, format text, and perform other superficial operations
...
\texttt{total\_biomass} module to the set of default soybean modules.
Afterwards, we can extract the final biomass for 2002 as predicted by the FvCB
model. Since we will be extracting the final biomass from the results of
multiple simulations during this analysis, we will create a reusable function
for doing this.
...
Note that we also replace the default canopy photosynthesis module in addition
to adding the \texttt{total\_biomass} module (as before). The return value from
\texttt{partial\_run\_biocro} is a function that we have called
...
we create a function that accepts the results of a BioCro simulation, extracts
the end-of-season biomass, and calculates the squared difference between
that result and the FvCB result. This will provide a means for associating an
error metric with each value of \ar.

<<fvcb_square_difference>>=
fvcb_square_difference = function(biocro_result) {
    (final_biomass(biocro_result) - final_biomass_fvcb_2002)^2
}
@
...
Here we define \texttt{f(x)} by applying \texttt{fvcb\_square\_difference} to
the output of \texttt{rue\_2002}.

<<alpha_rue_optimization>>=
rue_fvcb_square_difference <- function(x) {
  fvcb_square_difference(rue_2002(x))
}
...
optimization (\ar), defined a second function that determines a measure of error
from the result of one simulation (\texttt{fvcb\_square\_difference}), combined
these two functions into one that relates a value of \ar\ to a value of the
error (\texttt{fvcb\_square\_difference}), and passed that function to an
...
# Remove some weird row names that appear while creating data frames
rownames(fvcb_aq_scatter_2002) <- NULL
rownames(rue_aq_scatter_2002) <- NULL
...
and the \texttt{c3\_leaf\_photosynthesis} module defines $f(x)$.
...
Several different routes are possible when analyzing the sensitivity of a
model's output to a driver. Here we choose the instantaneous biomass as the
dependent variable, and we perturb the driver at all time points. For example,
in the case of air temperature, we perturb the air temperature at every time
point, run the simulation, and calculate the resulting change in biomass at
every time point.
