...
alpha_rue_sequence = seq(0.02, 0.03, length=31)
...
examining the relationship between net \cotwo\ assimilation rates ($A_n$) and
...
base names of \texttt{incident\_ppfd} ($Q$) and \texttt{Assim} ($A_n$),
for example \texttt{sunlit\_incident\_ppfd\_layer\_0} and
\texttt{shaded\_Assim\_layer\_9}. To help with plotting, it is beneficial
to first extract all the ($Q$, $A_n$) pairs from each layer and leaf class in
...
Now it is possible to produce scatter plots of ($Q$, $A_n$) values for each
...
in the BioCro II paper, although the paper uses gross assimilation rather than
net assimilation). Note: because the canopy model uses ten layers and two
leaf classes, there are twenty ($Q$, $A_n$) pairs per time point; because the
crop's state is stored at hourly intervals over a growing season that lasts
for 137 days, there are 3,288 time points, yielding 65,760 ($Q$, $A_n$) pairs in
...
fvcb_aq_scatter_plot <- xyplot(
    ...
    grid = TRUE,
    main = '(An, Q) scatter plot from the FvCB model in 2002'
)
...
rue_aq_scatter_plot <- xyplot(
    ...
    ylab = 'Net CO2 assimilation rate (micromol / m^2 / s)',
    grid = TRUE,
    main = '(An, Q) scatter plot from the RUE model in 2002'
)
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{
      $A_n$ versus $Q$ for soybean leaves in each canopy layer (0-9) and class
...
calculate the net assimilation rate for different values of the independent
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{
      Normalized sensitivity coeffficients characterizing the response of $A_n$
