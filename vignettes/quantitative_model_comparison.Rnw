...
ensuring that we're using the desired value for the atmospheric [\cotwo]
concentration for 2002 by replacing the value contained in the
\texttt{soybean\_parameters} list with value obtained from the NOAA data.
...
fvcb_result_2002 <- run_biocro(
    ...
    within(soybean_parameters, {Catm = catm[['2002']]}),
    weather[['2002']],
...
photosynthesis module, add the total biomass module (as before), add a value for
the \ar\ parameter, and again, make sure we're using the correct value for the
atmospheric [\cotwo].
...
rue_fvcb_square_difference = function(alpha_rue) {
    rue_result <- run_biocro(
        ...
        within(soybean_parameters,
            {alpha_rue = alpha_rue; Catm = catm[['2002']]}),
        weather[['2002']],
...
Here the \texttt{optim} function is used, but this design allows any optimizer
available in R to be used. For example, one could easily adapt this to search
for parameters using simulated annealing or evolutionary algorithms. This is in
contrast to some dynamical system software programs, in particular GUI-driven
programs, which offer a limited and nonextendable number of choices for these
algorithms.
...
optimal_rue_result_2002 <- run_biocro(
    soybean_initial_values,
    within(soybean_parameters,
        {alpha_rue = best_alpha_rue; Catm = catm[['2002']]}),
    weather[['2002']],
    append(within(soybean_direct_modules,
        {canopy_photosynthesis = "ten_layer_rue_canopy"}), "total_biomass"),
    soybean_differential_modules,
...
biomass_temp_sensitivity_rue <- biomass_driver_sensitivity(
    "temp",
    within(soybean_parameters, {alpha_rue = best_alpha_rue; Catm = catm[['2002']]}),
