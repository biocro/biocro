...
rue_2002 <- partial_run_biocro(
    ...
    append(within(soybean_direct_modules,
        {canopy_photosynthesis = 'std_lib:ten_layer_rue_canopy'}), 'std_lib:total_biomass'),
    soybean_differential_modules,
    soybean_ode_solver,
...
)
{
    ...
    gross_assim_center <-
        evaluate_module('std_lib:c3_leaf_photosynthesis', base_module_inputs)[['GrossAssim']]
    ...
    gross_assim_neg <-
        evaluate_module('std_lib:c3_leaf_photosynthesis', neg_module_inputs)[['GrossAssim']]
    ...
    gross_assim_pos <-
        evaluate_module('std_lib:c3_leaf_photosynthesis', pos_module_inputs)[['GrossAssim']]
...
biomass_temp_sensitivity_fvcb <- biomass_driver_sensitivity(
    'temp',
    soybean_parameters,
    'std_lib:ten_layer_c3_canopy'
)

biomass_temp_sensitivity_rue <- biomass_driver_sensitivity(
    'temp',
    within(soybean_parameters, {alpha_rue = best_alpha_rue}),
    'std_lib:ten_layer_rue_canopy'
)
@
...
rue_result_2006 <- run_biocro(
    ...
    weather[['2006']],
    within(soybean_direct_modules,
        {canopy_photosynthesis = 'std_lib:ten_layer_rue_canopy'}),
    soybean_differential_modules,
    soybean_ode_solver
)
@
