...
models for one year. Our example problem can be considered a trivial case of
...
fvcb_result_2002 <- run_biocro(
    ...
    soybean_parameters,
    weather[['2002']],
    append(soybean_direct_modules, std_lib('total_biomass')),
...
rue_2002 <- partial_run_biocro(
    ...
    append(within(soybean_direct_modules,
        {canopy_photosynthesis = std_lib('ten_layer_rue_canopy')[[1]]}),
        std_lib('total_biomass')),
    soybean_differential_modules,
    soybean_ode_solver,
...
we create a function that accepts a value of \ar, passes it to
...
Here we pass the \texttt{rue\_fvcb\_square\_difference} function to
\texttt{optim} in order to find the best value for \ar.
...
{
    photo_module <- std_lib('c3_leaf_photosynthesis')

    var_center <- base_module_inputs[[varname]]
    gross_assim_center <-
        evaluate_module(photo_module, base_module_inputs)[['GrossAssim']]

    neg_module_inputs <- base_module_inputs
    neg_var <- base_module_inputs[[varname]] * (1 - relative_perturbation)
    neg_module_inputs[[varname]] <- neg_var
    gross_assim_neg <-
        evaluate_module(photo_module, neg_module_inputs)[['GrossAssim']]

    pos_module_inputs <- base_module_inputs
    pos_var <- base_module_inputs[[varname]] * (1 + relative_perturbation)
    pos_module_inputs[[varname]] <- pos_var
    gross_assim_pos <-
        evaluate_module(photo_module, pos_module_inputs)[['GrossAssim']]
...
biomass_temp_sensitivity_fvcb <- biomass_driver_sensitivity(
    'temp',
    soybean_parameters,
    std_lib('ten_layer_c3_canopy')[[1]]
)

biomass_temp_sensitivity_rue <- biomass_driver_sensitivity(
    'temp',
    within(soybean_parameters, {alpha_rue = best_alpha_rue}),
    std_lib('ten_layer_rue_canopy')[[1]]
)
@
