...
Section 3 of the BioCro II paper (accepted by \emph{in silico Plants}; citation
forthcoming). Briefly, this section of the paper compares two different models
...
\texttt{BioCro:total\_biomass} module to the set of default soybean modules.
...
for doing this. Note that we also define a set of soybean parameters specialized
for Champaign, IL that includes clay loam soil parameters.

<<fvcb_result_2002>>=
cmi_soybean_parameters <- c(soybean_parameters, soil_parameters$clay_loam)

fvcb_result_2002 <- run_biocro(
    soybean_initial_values,
    cmi_soybean_parameters,
    weather[['2002']],
    append(soybean_direct_modules, 'BioCro:total_biomass'),
...
to adding the \texttt{BioCro:total\_biomass} module (as before). The return
...
rue_2002 <- partial_run_biocro(
    soybean_initial_values,
    within(cmi_soybean_parameters, {alpha_rue = NA}),
    weather[['2002']],
    append(within(soybean_direct_modules,
        {canopy_photosynthesis = 'BioCro:ten_layer_rue_canopy'}), 'BioCro:total_biomass'),
    soybean_differential_modules,
    soybean_ode_solver,
...
optimal_rue_result_2002 <- run_biocro(
    soybean_initial_values,
    within(cmi_soybean_parameters, {alpha_rue = best_alpha_rue}),
    weather[['2002']],
    append(within(soybean_direct_modules,
        {canopy_photosynthesis = 'BioCro:ten_layer_rue_canopy'}), 'BioCro:total_biomass'),
    soybean_differential_modules,
...
incident_par <- incident_ppfd * cmi_soybean_parameters[['par_energy_content']]

# Determine the corresponding incident shorwave values using the fraction of
# solar energy that lies in the PAR band (J / m^2 / s)
incident_shortwave <- incident_par / cmi_soybean_parameters[['par_energy_fraction']]
...
average_absorbed_shortwave <-
    incident_shortwave *
    (1 - cmi_soybean_parameters[['leaf_reflectance']] -
        cmi_soybean_parameters[['leaf_transmittance']]) /
    (1 - cmi_soybean_parameters[['leaf_transmittance']])
...
)
{
    ...
    gross_assim_center <-
        evaluate_module('BioCro:c3_leaf_photosynthesis', base_inputs)[['GrossAssim']]

    neg_inputs <- base_inputs
    neg_var <- base_inputs[[varname]] * (1 - relative_perturbation)
    neg_inputs[[varname]] <- neg_var
    gross_assim_neg <-
        evaluate_module('BioCro:c3_leaf_photosynthesis', neg_inputs)[['GrossAssim']]

    pos_inputs <- base_inputs
    pos_var <- base_inputs[[varname]] * (1 + relative_perturbation)
    pos_inputs[[varname]] <- pos_var
    gross_assim_pos <-
        evaluate_module('BioCro:c3_leaf_photosynthesis', pos_inputs)[['GrossAssim']]
...
for (varname in fvcb_light_curve_sensitivity_variables) {
    fvcb_sensitivity_light_curve_result[[varname]] <-
        apply(light_curve_inputs, 1,
            function(x) assim_sensitivity(varname, c(cmi_soybean_parameters, as.list(x))))
}
@
...
)
{
    ...
    neg_parameters[[varname]] <- parameters[[varname]] * (1 - relative_perturbation)
    neg_result <- run_biocro(
        soybean_initial_values,
        neg_parameters,
        weather[['2002']],
    ...
    pos_parameters[[varname]] <- parameters[[varname]] * (1 + relative_perturbation)
    pos_result <- run_biocro(
        soybean_initial_values,
        pos_parameters,
        weather[['2002']],
...
fvcb_result_2006 <- run_biocro(
    soybean_initial_values,
    within(cmi_soybean_parameters, {Catm = catm[['2006']]}),
    weather[['2006']],
...
rue_result_2006 <- run_biocro(
    soybean_initial_values,
    within(cmi_soybean_parameters, {alpha_rue = best_alpha_rue; Catm = catm[['2006']]}),
    weather[['2006']],
    within(soybean_direct_modules,
...
for (i in seq_along(years)) {
    # Run the RUE soybean model for this year, adding the
    # 'BioCro:total_biomass' module to the set of default modules and ensuring
    # that we're using the correct value for the atmospheric CO2 concentration
    rue_result <- run_biocro(
        soybean_initial_values,
        within(cmi_soybean_parameters,
    ...
    # Run the FvCB soybean model for this year, adding the
    # 'BioCro:total_biomass' module to the set of default modules and ensuring
    # that we're using the correct value for the atmospheric CO2 concentration
    fvcb_result <- run_biocro(
        soybean_initial_values,
        within(cmi_soybean_parameters, {Catm = catm[[years[i]]]}),
        weather[[years[i]]],
        append(soybean_direct_modules, 'BioCro:total_biomass'),
        soybean_differential_modules,
        soybean_ode_solver
