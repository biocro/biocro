...
that we also define a soybean model specialized for Champaign, Illinois that
includes clay loam soil parameters and an additional module
(\texttt{BioCro:total\_biomass}) that is not part of the standard soybean model;
as its name suggests, this module simply calculates the total biomass by adding
the separate tissues together, and including it simplifies later calculations.
...
for (varname in fvcb_light_curve_sensitivity_variables) {
  fvcb_sensitivity_light_curve_result[[varname]] <-
    apply(light_curve_inputs, 1,
      function(x) assim_sensitivity(varname, c(cmi_soybean$parameters, as.list(x))))
}
@
