...
list contains growing seasons of weather data from Champaign, IL. Full years of
weather data are included in the BioCro package as the \texttt{weatherYYYY}
datasets, where \texttt{YYYY} is a four-digit year between 1995 and 2020. For
this analysis, photoperiod length as determined from the soybean circadian clock
model is added to the original weather data, and then the full yearly set is
truncated to days 152 - 288.
...
model's output to a driver. Here we choose to instantaneous biomass as the
dependent variable, and we perturb the driver at all time points. I.e., in the
