...
cable(result[43198:43202,]) # around mid day
...
Thus, the way we have been using the `time` variable in the previous
examples in inconsistent with the usual interpretation of time in two
respects: First, the units were in seconds rather than days.  Second,
the time represented the amount of time elapsed from the beginning of
the simulation rather than an "absolute" time, that is, a time value
specifying a specific time of day on a specific day of the year.
...
by 86400.  Now, the value of `time` increases by 1/86400 (days!) from
row to row (an amount still corresponding to one second):
...
its usual interpretation.  But the `time` value of 0 corresponds to
midnight at the start of December 31, and this is not consistent with
a day having 12 hours of daylight unless we are at an equatorial
location.  We may remedy this by shifting the time values forward by
79 days to the time of the spring equinox.  Now time represents not
...
used if we wished.  (We still can not mix in derivative modules such
as the thermal time modules that use `time` as input, however, since
