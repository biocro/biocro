...
case where the function domain $U$ isn't a set of intergers at all but
...
* The _parameters_ correspond to the sole value in the codomain of the
  ...
  (A special case is the parameter `timestep`, which must always be
  included among the BioCro parameters, but which does not naturally
  belong among the bona fide parameters belonging to the set $K$.  We
  discuss `timestep` further in section \ref{sec:biocro_time}.)
...
\end{equation}
                              <!-- (no par break) -->
where $\vs{W}_i$ is the subspace of $\vs{W}$ generated by the variables in the
output of module $\M_i$ and $\vs{\overline{W}}_i$ is the subspace of $\vs{W}$
generated by those inputs to module $\M_i$ that are not in $X$, $K$, or $D$.  (A
direct module's inputs and outputs are always disjoint so that
$\vs{\overline{W}}_i$ is complementary to $\vs{W}_i$, with $\vs{W}_i \times
\vs{\overline{W}}_i$ a subspace of $\vs{W}$.)  We will discuss this further in
the section on modularization.
...
interface, there are always three additional quantities---`doy`,
`hour`, and `time`---which always come into play when we set up or run
a system.  These are artifacts of certain aspects of the R interface
and are subject to revision, and we will mostly ignore them here.)
...
presented in chapter \ref{sec:formulations}.
...
character from the other parameters the make up part of the
...
modeled mathematically.  As such, we don't consider it as one of the
variables in the set $K$ that we introduced in section
\ref{sec:khalil_elaboration}.
