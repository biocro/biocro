...
predicting its path as it falls through the air. We may find that
in order to accurately model the free-fall of a feather, we _do_
need to include attributes in our system that represent the impact
of air resistance. This process, where a model is compared against
observations and then updated when it fails to predict the behavior
of the real system it is meant to represent, is a key part of the
scientific process and has been responsible for a great deal of
progress and understanding. Thus, it is never a problem when the
process of abstraction has left out too many details; instead this
is an opportunity to learn more about the real world and the model.
A central goal for BioCro is to make it relatively easy to add new
components to an existing model, allowing this type of model
development to occur more rapidly.
...
\end{align*}
                              <!-- (no par break) -->
Thus, Equation \eqref{eq:falling_body_recursion} may seem like a complicated way
to write a relatively simple rule relating height, velocity, and acceleration.
However, this notation will become useful later when we begin to consider
general systems where we do not wish to specify a particular number of state
variables, a particular rule for determining a future state, or even the
independent variable (which may not be time).
...
\end{equation*}
                              <!-- (no par break) -->
One important point is that the process of developing a recursive
...
method, we would arrive at a recursive definition different from
Equation \eqref{eq:falling_body_recursion}. Nevertheless, it would
represent the same continuous system, and should (usually) produce
a similar sequence of states.
...
hand, more realistic biological models tend to be non-autonomous or
...
Aside from differences in the concept of _state_ (Section \ref{sec:state}), the
Khalil model fits very well with an idealized version of the BioCro model in
which time is considered to be continuous.  We will see this in the section
\ref{sec:BioCro_systems}, where we discuss the two models side-by-side. But
first, we must elaborate a bit further on the Khalil model.
...
allowing the state to evolve from its initial values) requires the
use of numerical methods that effectively discretize the continous
system of differential equations of a Khalil system. Whether this is
considered as the development of a new discrete dynamical system from
a continuous one, or just a matter of practical convenience, is mostly
a philosophical issue that we will not discuss any further.[^discrete_biocro]
...
the section on modularization (Section \ref{sec:modularization}).
...
* The _differential module_ corresponds to the function $\vec{f}$ in
  ...
  are determined by the initial values as discussed above.
...
commonly used in dynamical systems theory, and in particular, with
the formulations presented in chapter \ref{sec:formulations}. Elsewhere
in this document, we endeavor to use the term _state_ exclusively in
reference to its usual definition as employed in the Khalil, Giuli-Mazzola,
and Barreira-Valls models; we intentionally avoid using BioCro's idea of
state, preferring to write _differential quantities_ instead to prevent
confusion. In this section, we must necessarily use _state_ to refer to
both conceptions, and try to clarify which definition is meant whenever
it is unclear from context.
...
As we demonstrate here, a BioCro system having drivers but not
including a time-like variable among its differential quantities
does not, in general, conform to the Giunti model.
...
The upshot is that the Giunti model does not naturally describe
BioCro systems unless some proxy for time is allowed to be a
component of the differential quantities. It should also be noted
that BioCro does not make use of two important generalizations in
the Giunti model: in BioCro, the state space always _will_ be a
subset (in fact, a _connected_ subset) of a Euclidean space, and
state transitions (using _state_ in Khalil's sense) will always be
differential equation based. Thus, we find that Khalil's model
provides a better description of BioCro systems.
...
derivatives. This operation is performed by the combined derivative function.
...
subdivided into two or more submodules. In this section, we will show that the
ordered sum of two modules is itself a module, which is the key to understanding
how to modularize the direct module function.
...
At this point, it is possible to see how the ordered sum of
two modules is itself a module; the key is to think in terms of
...
all the outputs $D$ of module $\M_j$. We can also describe this more
formally, as will be shown in the remainder of this section.
...
[^discrete_biocro]: There are rare exceptions to this rule because it
is possible to write BioCro modules that assume discrete time steps; in
other words, modules that require the use of an Euler solver. In this
case, the discrete-time system is _not_ an abstraction of a continuous
system. However, such modules are avoided in "modern" BioCro systems,
and most BioCro systems can be considered as continuous-time models
that are discretized as a practical necessity.
