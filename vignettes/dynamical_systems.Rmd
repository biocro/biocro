...
\begin{align}
  h(0)         & = h_0  \notag \\
  h'(t)|_{t=0} & = 0    \label{eq:ode-system} \\
  h''(t)       & = -g   \notag
\end{align}
                              <!-- (no par break) -->
...
Anyone who has studied differential equations will immediately
recognize this as an initial-value problem, one whose unique solution
is
                              <!-- (no par break) -->
\[
  h(t) = h_0 - 1/2\,gt^2.
...
nutrient or energy content of the growing plant; the impact of a group
of plants on the surrounding environment, including the temperature
and $\text{CO}_\text{2}$ content of the surrounding air, or the rate
of erosion of the soil substrate; or the resilience of the plant in
drought conditions.
...
weight of a compact and relatively dense object has little impact upon
...
rather good in the case of compact, relatively-dense objects.  (It's
not so good for feathers.)
...
variable_ to mean any quantifiable attribute of a system; the _state_
of a system is then the conglomeration of the values of all of these
variables at any particular time.  Euler's method makes the assumption
that, given a known state of a system at some particular time, the
state of the system at some very small interval of time later can be
...
In the system of equations at \eqref{eq:ode-system}, $h$ is the only
...
of the state.  If we think of the state of a system as a snapshot of
the system at a particular time, this may seem somewhat
counter-intuitive.  While it is clear how we might consider the height
of the object as part of that snapshot, if we take the notion of
"snapshot" in its literal sense, it may seem odd to consider the
velocity, the rate of change of position, as part of a snapshot: when
we take a literal snapshot, we can't know how fast the objects in the
snapshot are moving, at least not if the snapshot is instantaneous.
We won't dwell on this philosophical problem: if it bothers you,
imagine that the object has a speedometer attached and that our
snapshot shows the position of its needle as the object is falling.
...
\end{align}
                              <!-- (no par break) -->
Now we can write a recursive definition for the state $\vec{s}$ as a function of $t$:
                              <!-- (no par break) -->
\begin{align}
  \vec{s}(t_0) &= (h_0, 0) \notag \\
...
\]
                              <!-- (no par break) -->
It should be pointed out, however, that not all discrete-time abstract
dynamical systems arise as abstractions of real systems or even as
abstractions of continuous-time abstract systems.
...
abstract dynamical system, along with discussion of how BioCro's
terminology may differ in sometimes unexpected ways from terminology
the reader may be used to.
...
$f(c)$ is defined for every $c\in C$, but in what follows, we
occasionally won't always be entirely strict about this.  Following
@vaught, we write $B^C$ to denote the set of all such functions
$f$.[^sets_of_functions] We use $\R$, $\Z$, and $\N$ to denote the
real numbers, the integers, and the natural numbers (the finite
ordinals, including zero), respectively.  $\Z^+$ denotes the
_positive_ integers.
...
vector $\vec{v}=(v_0, v_1, v_2, \dots, v_{n-1})$.  Often, however,
there will be some level of indirection involved in our use of the
notation $v_i$ for a coordinate of $\vec{v}$.  For example, if
$\vs{U}$ is a proper subspace of $\R^n$, so that $\vs{U} = \R^U$ for
some proper subset $U$ of $n = \{0,1,\dots,n-1\}$, then we may regard
$u_i$ as the value taken by the $i$th member of $U$ in some arbitrary
but fixed ordering of the members of $U$.  We even allow the case
where the function domain $U$ isn't a set of intergers at all but just
some finite collection of objects.  In this case, in the context of
considering vectors (_qua_ mappings) $\vec{v}\in \R^U$, $v_i$ may
denote alternately the i'th member of $U$ in some fixed enumeration;
the name of a variable associated with that member; or the value of
the i'th coordinate of some particular vector $\vec{v}$.  In the later
case, $v_i$ doesn't abbreviate $v(i)$ for some function $v\in \R^U$.
Rather, it stands for $v(u_i)$, where $u_i$ denotes "the i'th member
of $U$."
...
literal union of $f$ and $g$.  Also, clearly,
                              <!-- (no par break) -->
\begin{align*}
    f &= (f\cup g)|X \\
    \intertext{and}
...
\]
                              <!-- (no par break) -->
If $A=\R^X$ and $B=\R^U$ with $X\cap U=\emptyset$, then there is a
natural isomorphism between $\R^X\times\R^U$ and $\R^{X\cup U}$ given
by
                              <!-- (no par break) -->
\[
    (\vec{x}, \vec{u}) \mapsto \vec{x}\cup\vec{u}.
\]
(The inverse mapping is given by $\vec{v} \mapsto (\vec{v}|X,
\vec{v}|U)$ for $\vec{v}\in \R^{X\cup U}$.)
                              <!-- (no par break) -->
Where convenient and warranted, we will consider $\R^X\times\R^U$ and
$\R^{X\cup U}$ identical.
...
of systems BioCro deals with at the systems level.
...
### The notion of state in the Khalil and BioCro models

Khalil remarks that the state variables "represent the memory that the
dynamical systems has of its past."  As Laplace
...
In BioCro, the inputs are referred to as _drivers_.  It should also be
remarked that in the terminology used to discuss BioCro, and in the
terminology of the software itself, the driver variables are
considered a part of the state.  This is partly for expedience: in
certain contexts, it is convenient to treat all quantities that vary
with time (and sometimes even those that don't) uniformly.

But convenience aside, there is also some philosophical justification
...
data that would be required, it isn't even feasible to do so.  So in
BioCro, we regard them as a part of the system---as part of the
...
model controlled experiments, such as those carried out in a green
house; or to model "thought experiments" or "what if" scenarios: _What
would happen if we used the weather data from 2005 but assumed a much
higher $\text{CO}_\text{2}$ concentration?_
...
origin.  (The falling body system considered above was autonomous: it
doesn't matter precisely when the object is released, its subsequent
motion will follow the same pattern.)

With the exception of certain "toy" examples, such as those based upon
the harmonic oscillator module, the systems with which BioCro deals
will, on a conceptual level, almost invariably be non-autonomous or
time-varying.  This is largely due to the time-dependence of the
drivers: it matters whether we start our plants growing in March or in
May.  (Formally, however, as we shall see, a non-autonomous system can
be made into an automous one by introducing a time-related variable
into the state.)

We will discuss the Khalil model further we discuss the systems model
used by BioCro.
...
it generalizes the concept of time used for dynamical system from the
...
non-negative even integers.  This is one of the natural ways to view
time in BioCro systems: in an idealized view of BioCro-modeled
systems, time is continuous, but on a practical level, we compute the
behavior of such systems in discrete jumps.
...
fortiori, there is no longer any requirement that the state
transitions be differential equation based.  (In BioCro, however, no
use is made of this generalization: in BioCro, the state space always
_will_ be a subset (in fact, a _connected_ subset) of a Euclidean
space, and state transitions (using _state_ in Khalil's sense) will
always be differential equation based.)
...
except time."  We will elaborate on this question further in what
follows.
...
continuous time_" [@alma99954905266605899, Definition 1.7] in precisely the same way as Giunti
and Mazzola (for the case where the time model is $\R$ or $\R_{\geq
0}$)---that is, as a family of mappings indexed by time satisfying the
semi-group property.)
...
Aside from differences in the concept of _state_, the Khalil model
fits very well with an idealized version of the BioCro model in which
time is considered to be continuous.  We will see this in the Section
\ref{sec:BioCro_systems}, where we discuss the two models
side-by-side.  But first, we will elaborate a bit further on the
Khalil model.
...
\end{align*}
                              <!-- (no par break) -->
In terms of the individual components of $\vec{u}$, each component
$u_i$ of $\vec{u}$ can be expressed as a function $\gamma_i$ of $t$
or $\vec{x}$ or both:
                              <!-- (no par break) -->
\begin{align*}
  u_i &= \gamma_i(t), \\
...
\noindent
Thus $U=K\cup D\cup W$, allowing us to partition the vector space
$\vs{U}$ into corresponding sub-vector spaces $\vs{K}$, $\vs{D}$, and
$\vs{W}$; that is,
                              <!-- (no par break) -->
\[
...
* The _initial values_ correspond to the state $\vec{x}$ at some
  ...
  $\vec{x}_0 = (x_{0,0}, x_{1,0}, \dots, x_{n-1,0})$.  In BioCro, it is
  the _initial values_ specification that determines (in Khalil's
  terminology) which variables comprise the state, and the dimension
  of the state space $\vs{X}$ is equal to the number of variables in
  $\vec{x}_0$.
...
BioCro, _all_ quantities are considered to be a part of the state, and
the variables whose values are specified when providing the set of
initial values to a BioCro simulation are referred to as _differential
variables_.  As such, when we are referring to things using this
terminology, we will refer to $\vs{X}$ not as the state space, but as
the _differential component of the state space_.  In BioCro, it is the
_initial values_ parameter (and not, for example, the set of
differential modules) that determines which variables belong to the
...
in this definition.  This can be done by adding an additional case to
the above rule; namely,
                              <!-- (no par break) -->
\begin{equation}
...
\end{equation}
                              <!-- (no par break) -->
This makes $\gamma^\vs{D}_i(t)$ into a continuous piece-wise linear
function.)

Now to the modules.  As we will show below, any BioCro system is
equivalent to a BioCro system having only a single module of each
type, but having the same initial values, parameters, and
...
type.[^single_module] Later, in the section on modularization, we will
consider how a single module may be broken up into multiple modules.
...
* The _direct module_ corresponds to the function $\gamma^\vs{W}:
  ...
  $\vs{U}$.  We call these variables the _direct_ quantities of the
...
* The _differential module_ corresponds to the function $\vec{f}$ in
  ...
  differential module is that its output variables must all be (in the
  terminology of Khalil) _state_ variables (as determined, you may
  recall, by which variables are included in the specification of the
  initial state).
...
parameters when setting up a system.  (In the R interface, there are
always three additional quantities---`doy`, `hour`, and `time`---which
always come into play when we set up or run a system.  These are
artifacts of certain aspects of the R interface and are subject to
revision, and we will mostly ignore them here.)
...
of four categories: it is either a constant; a driver; a differential
quantity; or a derived quantity, that is, a quantity whose value at
any given time can be directly computed from the values of other
quantities at that same time using some more or less simple formula.
(These correspond, respectively, to the subspaces $\vs{K}$, $\vs{D}$,
$\vs{X}$, and $\vs{W}$ discussed in Sections
...
_state_ of the system at that time.  This section attempts to
reconcile this conception of state with that commonly used in
dynamical systems theory, and in particular, with the formulations
presented in section \ref{sec:formulations}.

### BioCro state and the Giunti model

A BioCro system having drivers but not including a time-like variable
in its state does not, in general, conform to the Giunti model.
...
I bring this up to show that even in the presence of drivers
...
from one day to the next.  And so to will the evolution of a generic
...
The upshot is that if we want to keep the Giunti model as a model for
BioCro systems, the best plan is to allow time or some proxy for time
to be a component variable of the state.
...
I use "merely" advisedly here, because one of the main strengths of
...
variable.  Therefore, we can rewrite equation
\eqref{eq:BioCro_state_equation} as
                              <!-- (no par break) -->
\begin{equation}
...
subdivided into two or more submodules, and, as also mentioned in
Section \ref{sec:BioCro_systems}, when a system has more than one
direct module, each constituent module $\M_i$ corresponds to a
function
                              <!-- (no par break) -->
\begin{equation}
  \gamma^{\vs{W}_i}: \vs{X}\times\vs{K}\times\vs{D}\times\vs{\overline{W}}_i \to \vs{W}_i,
...
never share inputs and outputs.  Note that it may be the case that
$\In\M_i\subseteq H$; in this case $\vs{\overline{W}}_i$ is
0-dimensional and \ref{eq:dir_mod_fn} reduces to
                              <!-- (no par break) -->
\[
...
Then we define $\M_i + \M_j$, the _ordered sum of $\M_i$ and $\M_j$_,
...
\ref{eq:ordered_sum_function} and \ref{eq:ordered_sum}: the domain of
$f\ast g$ being $\R^{ A\cup (C\smallsetminus B)}$ corresponds to the
inputs for the corresponding module being $A\cup (C\smallsetminus B)$,
which is just $\In\M_i \cup (\In M_j\smallsetminus \Out\M_i)$.
...
Suppose we are given some value $x$ in $\R^{ A\cup (C\smallsetminus
B)}$, the domain of $f\ast g$.  We can describe $(f\ast g)(x)\in\R^{
B\cup D}$ by describing the way to compute how $(f\ast g)(x)$ maps
each $y\in B\cup D$ into $\R$.
...
A\cup (C\smallsetminus B)\to A}(x)$ of $x$ to $\R^A$ tells us that we
need consider only the coordinates of $x$ that correspond to members
of $A$.  Once we have a vector in $\R^A$, we can apply the function
$f$ to obtain a value in $\R^B$.  This is all we need, since $y$ is in
$B$.
...
(such as $y$) in $D$.  But $x$ is in $\R^{A\cup(C\smallsetminus B)}$,
so $x$ only tells how values in $C$ that aren't also in $B$ are
mapped.  The mapping for these values corresponds to the projection
$\pi^{ A\cup (C\smallsetminus B) \to C\smallsetminus B}(x)$, a value
in $\R^{C\smallsetminus B}$.  To find the portion of the mapping we
need that belongs to $\R^{C\cap B}$, we look at $\pi^{ B\to C\cap
B}\circ f\circ \pi^{ A\cup (C\smallsetminus B)\to A}$.  As we have
just seen, $f\circ \pi^{ A\cup (C\smallsetminus B)\to A}$ maps $x$ to
a member of $\R^B$.  Then we can apply the projection $\pi^{ B\to
C\cap B}$ to obtain a member of $\R^{C\cap B}$.  Taking the union of
the components in $\R^{C\smallsetminus B}$ and $\R^{C\cap B}$ gives us
a value in $\R^C$, to which we can apply function $g$.  The result is
a function in $\R^D$ telling how all values (such as $y$) in $D$ are
mapped.

This is all perhaps easier to understand if we think in terms of
module inputs and outputs: Given mappings for all values in $A$ and
all values in $C$ that aren't in $B$, we can obtain mappings for all
values in $B$ and $D$ as follows: Since we know all the inputs $A$ to
module $\M_i$, we can use the module to obtain all the outputs $B$.
Now we know all the inputs $C$ to module $\M_j$---both those that
_aren't_ in $B$ (which were given at the outset), and those that _are_
in $B$ (which were obtained by applying module $\M_i$).  This yields
all the outputs $D$ of module $\M_j$.
...
both `doy` and `hour`.  All three variables show up in the output.
...
and `hour` must be; if only one is, we get obscure error:
...
isomophic to $\R^p$.  In many contexts, we could just go ahead and
consider them not just isomophic but identical.  But here we want to
be more careful because we want to be able to simultaneously consider
functions into $\R$ having non-overlapping domains.
\par
Note that in practice, in a model of a real-world system, $\vec{f}$
may not be defined for all $\vec{x}\in\vs{X}$ and all
...
[^giunti]: I shall henceforth refer to the model from @Giunti&Mazzola
...
[^elevation]: Here, I am using _altitude_ to mean "height above sea
