...
\begin{itemize}
  ...
  a public GitHub repository: \url{https://github.com/ebimodeling/biocro}.

  \begin{itemize}
    \item Automated tests ensure that the code does not break following any
    changes.

    \item The public version on GitHub is BioCro \texttt{2.0.0}, but today we
    will be using BioCro \texttt{3.0.0}.
  \end{itemize}
...
\begin{itemize}
  ...
  \href{https://github.com/ebimodeling/biocro}{GitHub repository} or via
  the annotated
  \href{https://ebimodeling.github.io/biocro-documentation/doxygen_docs_modules/annotated.html}{Doxygen documentation}.
  For example:

  \begin{itemize}
    \item A short description of the \texttt{c3\_assimilation} module can be
    found on its associated
    \href{https://ebimodeling.github.io/biocro-documentation/doxygen_docs_modules/classc3__assimilation.html#details}{class overview page}
    in the Doxygen documentation.

    \item The units for its inputs and outputs can be found as comments in its
    source code, available on its associated
    \href{https://ebimodeling.github.io/biocro-documentation/doxygen_docs_modules/c3__assimilation_8h_source.html}{source code page}
...
evaluate_module(
  ...
  within(soybean$parameters, {
    Qp = 1800      # incident PPFD in micromol / m^2 / s
...
light_response_curve <- module_response_curve(
  ...
  data.frame(Qp = seq(0, 2000, length.out = 101))
)
@

\noindent The return value is a data frame (an R structure equivalent to a
table), and the response of net assimilation to incident light intensity can be
...
\begin{itemize}
  ...
  assimilation rate from inputs like the incident light intensity and
...
\noindent The data is stored in \texttt{csv} files and can be loaded into the R
...
temperature_response_curve <- module_response_curve(
  'BioCro:c3_assimilation',
  within(soybean$parameters, {Qp = 2000; StomataWS = 1}),
  expand.grid(
...
caption <- paste(
  ...
  unique(temperature_response_curve$Qp),
  'micromol / m^2 / s\nusing the',
...
\noindent \textbf{Exercise}: Try changing the value of the incident
photosynthetically active photon flux density \texttt{Qp} and notice how the
...
optim_temp_plot <- xyplot(
  ...
  main = 'Optimizing C3 photosynthesis when\nQp = 2000 in the absence of water stress'
)
...
an option to specify the incident light level. First, we'll need to create a
...
Now we will define a range of incident light values to use and calculate the
sensitivity coefficients for each one using a base level of $C_a$ = 400 ppm.

<<sensitivity_calculations>>=
qp_seq <- seq(0, 2000, length.out = 101)

sens_result <- do.call(rbind, lapply(qp_seq, function(Qp) {
  sens <- sensitivity(
    function(catm) {co2_func(c(catm, Qp))$outputs$Assim},
    400
  )
  sens$Qp <- Qp  # Also include the incident light level in the result
  return(sens)
}))
@

Now we can see the sensitivity of net assimilation to ambient CO$_2$
concentration at different levels of incident light intensity.
...
sensitivity coefficient is non-negative for all values of incident light; this
...
increases in ambient CO$_2$ levels when the incident light levels are high. This
...
photosynthesis component uses the incident sunlight level and the leaf mass to
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{
      ...
      incident sunlight and total leaf mass are used to determine the rate of
...
kg s$^{-1}$) from the incident photosynthetically-active photon flux density
...
\end{subequations}
%
We would like to consider the incident sunlight $Q$ as varying with time, so we
...
conceptual diagram within the equations. For example, the incident sunlight $Q$
...
spaced values of incident light (\texttt{Qp}) between 0 and 2000 $\mu$mol
