...
process limits carboxylation for all realistic values of $C$. Inspecting the
equation, we can see that there are two ways for it to become negative. One is
for the denominator to be negative, which happens when $4 \cdot V_{cmax} < J$.
This is a situation where $V_{cmax}$ is small and carboxylation is always
RuBP-saturated. On the other hand, the numerator becomes negative when
$J < 8 \cdot V_{cmax} / \left[ K_c \cdot \left( 1 + O / K_o \right) \right]$.
This is a situation where $J$ is small and carboxylation is always
RuBP-regeneration-limited. Essentially, Equation \@ref(eq:cc) tells us that
there is no simple rule for determining which process limits carboxylation in
the FvCB model.
...
\begin{equation}
\left. A_n \right|_{C = 0} = \max \bigl\{ A_c \big|_{C = 0}, A_j \big|_{C = 0} \bigr\}.
\end{equation}
