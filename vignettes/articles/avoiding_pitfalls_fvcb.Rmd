...
RuBP-regeneration-limited. (Of course, $C_c$ will still be positive if both the
numerator and denominator are negative.) Even when $C_c$ is positive, it may not
be realistically obtainable; for example, if $J$ is just a tiny bit smaller than
$4 \cdot Vcmax$, $C_c$ can take very large values that would only be achievable
in a gas evironment of nearly pure CO~2~. Essentially, this analysis of Equation
\@ref(eq:cc) tells us that there is no simple rule for determining which process
limits carboxylation in the FvCB model, and it is even possible that a crossover
between limiting processes may not occur (see Figure \@ref(fig:cc)).
