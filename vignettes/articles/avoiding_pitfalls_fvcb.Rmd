...
response of plants to key environmental variables like incident light, CO~2~,
...
these assumptions and explain how to avoid them when understanding or using the
FvCB model.

# The Central FvCB Equation
...
In other words, the crossover point where photosynthesis starts to become a net
producer of carbon occurs when $C$ takes a special value called $\Gamma^*$
defined by:
\begin{equation}
...
The rest of the FvCB model consists of a method for calculating $V_c$, which
...
- Following a carboxylation or an oxygenation, each RuBP molecule must be
  ...
  found in many places, such as Equations 2.21 and 2.21 in
...
some threshold value of $C$. The special points $\Gamma^*$ and the CO~2~
compensation point are interesting; however, neither of these have any bearing
on which processes might be limiting the carboxylation rate. In fact, there is
simple counterexample demonstrating a situation where $W_j < W_c$ at low $C$:
darkness. Darkness ensures that $J = 0$ and hence $W_j \leq W_c$ for all values
of $C$.
...
$A_c = W_c \cdot \left( 1 - \Gamma^* / C \right)$ is also negative. Likewise,
$A_j$ is negative, but its magnitude is larger than $A_c$ because $W_c < W_j$.
So, in this case, $\min \{ A_c, A_j \} = A_j$. Thus, Equation \@ref(eq:analt)
has chosen the incorrect limiting factor. In fact, Equation \@ref(eq:analt)
_always_ chooses the wrong limiting factor when $C < \Gamma^*$.
...
model's true output in other conditions where $J$ is low.
...
Triose phosphate is the primary end product of photosynthesis; it is exported
from the chloroplast and eventually used to form monosaccharide sugars such as
glucose. When glucose is created, most of the phosphate from the triose
phosphate molecules is released back to the choloroplast. Thus, if triose
phosphate is not utilized quickly enough, chloroplast phosphate can become
depleted, placing a limitation on photosynthesis [@mcclain_triose_2019]. While
this limitation was not included in the original FvCB model, its importance was
realized soon afterwards [@sharkey_photosynthesis_1985; @harley_improved_1991]
and it has since become a standard part of the model.

Under this limitation, the rate of phosphate consumption due to photosynthesis
is equal to the rate of triose phosphate utilization (TPU), as expressed by
Equation 2.25 of @caemmerer_biochemical_2000:
\begin{equation}
\frac{V_c}{3} - \frac{\phi \cdot V_c}{6} - \frac{\alpha \cdot \phi \cdot V_c}{2} = T_p, (\#eq:tpu)
\end{equation}
where $0 \leq \alpha \leq 1$ is the fraction of glycolate carbon not returned to
the chloroplast, the left-hand side is the rate of phosphate consumption, and
$T_p$ is the TPU rate. Using the definition of $\phi$, Equation \@ref(eq:s), and
Equation \@ref(eq:gstar), we can replace $V_o$ in Equation \@ref(eq:tpu) by
$2 \cdot V_c \cdot \Gamma^* / C$ and solve for $V_c$ to find the TPU-limited
carboxylation rate, which is typically called $W_p$:
\begin{equation}
W_p = \frac{3 \cdot T_p \cdot C}{C - \Gamma^* \cdot \left( 1 + 3 \cdot \alpha \right)}. (\#eq:wp)
\end{equation}
Since we are now considering a new possible limitation in the model, we must
update Equation \@ref(eq:mincj) to include $W_p$:
\begin{equation}
...
to understand the situation is to set
$C = \Gamma^* \cdot \left( 1 + 3 \cdot \alpha \right)$ and evaluate the
left-hand side of Equation \@ref(eq:tpu), which represents the rate of phosphate
consumption from photosynthesis. Doing so, we find that this rate is exactly 0
when $C = \Gamma^* \cdot \left( 1 + 3 \cdot \alpha \right)$. In other words,
when $C$ is low enough, photosynthesis is not consuming _any_ phosphate, and it
is therefore impossible for carboxylation to be limited by TPU. One way to
indicate this restriction is to modify Equation \@ref(eq:wp) to the following:
\begin{equation}
  W_p =
  \begin{cases}
...
$C = 0$:
\begin{equation}
A_j (C = 0) = - \frac{\Gamma^* \cdot J}{8 \cdot \Gamma^*} - R_d
\end{equation}
and
\begin{equation}
A_c (C = 0) = - \frac{\Gamma^* \cdot V_{cmax}}{K_c \cdot \left( 1 + O / K_o\right)} - R_d.
\end{equation}
...
Now, we will define a function that implements the pseudo-FvCB model:
...
the FvCB model but assimilation is RuBP-saturated in the psudo-FvCB model.
