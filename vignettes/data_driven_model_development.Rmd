...
knitr::opts_chunk$set(echo=FALSE, comment = FALSE, warning = FALSE, echo = FALSE)
```
...
### Overall
V0.6_V0.91 <- t.test(results$V0.6, results$V0.91, paired = TRUE)

result.table <- data.frame(version = names(x), 
                      description = c("start", "added C3", "perenating stem", "growth respiration", "leaf senescence"), 
                      sd = round(sd.f, 2), RMSE = round(R, 2), P = round(c(NA, V0.7$p.value, V0.75$p.value, V0.81$p.value, V0.91$p.value), 2))
library(pander)
