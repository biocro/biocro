...
\texttt{module\_response\_curve} include \emph{all} of the module's outputs, we
could also use \texttt{rc} to create plots of the response of the stomatal
conductance to water (\texttt{Gs}) or the intercellular CO$_2$ concentration
(\texttt{Ci})).
