...
\newcommand{\differentialmemberfunction}{\ensuremath{\mathbf{g}_i}}
\newcommand{\differentialmodulefunction}{\ensuremath{\mathbf{g}_M}}
\newcommand{\directmemberfunction}{\ensuremath{\mathbf{f}_i}}
...
\newcommand{\statevectorexclusion}{\ensuremath{\mathbf{x}^i}}
...
function \directmodulefunction\ that maps its inputs to its outputs according to
...
\differentialmodulefunction\ that maps its inputs to derivatives of its outputs
...
Within the context of a simulation, certain considerations apply to the inputs
and outputs of modules to ensure that the system is well-defined and can in
principle be solved:
\begin{enumerate}

\item Since direct modules calculate instantaneous values of quantities, at most
one direct module can have a particular quantity as an output. For example,
...
differential modules can have a particular quantity as an output; in this
...
When these rules are satisfied, it is safe to assume that the inputs and outputs
of all modules are part of the state \statevector\ and that no direct modules
have any overlap between their inputs and outputs (since this would form a
cyclical dependency). Under these circumstances, we can say that the collection
of direct modules defines a collection of functions where each function
\directmemberfunction\ maps a subset of the state onto an individual state
element \statemember\ according to

\begin{equation}
\statemember = \directmemberfunction \left( \statevectorexclusion, t \right),
\label{eq:direct}
\end{equation}

\noindent where \statevectorexclusion\ represents the other state quantities
besides \statemember. Likewise, we can also say that the collection of
differential modules defines a collection of functions where each function
\differentialmemberfunction\ maps the state onto the derivative of an
individual state element \statemember\ according to

\begin{equation}
\frac{d \statemember}{dt} = \differentialmemberfunction \left( \statevector, t \right).
...
simulation) and the \emph{initial values} of the differential quantities
(Section \ref{sec:choose_modules}). In general, the sets of required parameters
and initial values will be determined by the modules and drivers that a user has
chosen to use (Sections \ref{sec:choose_modules} and \ref{sec:choose_drivers},
respectively).
...
function from the \texttt{lattice} package, which must be loaded into the R
