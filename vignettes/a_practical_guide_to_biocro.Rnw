...
% error=FALSE: If we can't load the needed libraries, we want to stop
...
\ref{sec:module_libraries}); each module and library has a name, allowing
individual modules to be specified by specially-formatted strings of the type
\newline \texttt{library\_name:module\_name}, where the library name
(\texttt{library\_name}) and module name (\texttt{module\_name}) are separated
by a colon (\texttt{:}). Collections of modules (which may come from multiple
libraries) can be specified in R using the \texttt{c} command, which creates an
R vector from its input arguments. For example, if we wish to use a module
called \texttt{Module\_1} from a library called \texttt{libA} and a module
called \texttt{Module\_2} from a library called \texttt{libB}, we could write
the following:

<<example_module_vector>>=
module_specifications <- c(
    'libA:Module_1',
    'libB:Module_2'
)
@
...
can retrieve or modify the first element of \texttt{module\_specifications} as
in the following example, where we replace \texttt{Module\_1} with
\texttt{Module\_3} from the same library:

<<example_vector_access>>=
print(module_specifications[1])
module_specifications[1] <- 'libA:Module_3'
print(module_specifications)
@
...
\texttt{differential\_module\_specifications}; it is \emph{not} the name or
specification of a module. Its purpose is to allow us to access that list
element by a meaningful identifier (\texttt{thermal\_time\_module}) rather than
its numbered position within the list. Now, if we want to use a different
equation for calculating thermal time, we can easily switch it via a command
like:

<<example_module_swap>>=
differential_module_specifications$thermal_time_module <- 'BioCro:thermal_time_trilinear'
@
...
a set of module specifications. Doing so can be avoided by using the
\texttt{module\_paste} function, which automatically prepends a library name to
a vector or list of module names, forming a set of full module specifications.
For example, we can create the same list of differential module specifications
using \texttt{module\_paste} as follows:

<<example_module_paste>>=
differential_module_specifications <- module_paste('BioCro', list(
  'partitioning_growth',
...
\newline \texttt{soybean\_direct\_modules} and
\texttt{soybean\_differential\_modules}. As discussed in that section, these
...
\texttt{soybean\_ode\_solver}. Its contents can be viewed as with any other
list:
...
soybean_result = run_biocro(
  ...
  soybean_parameters,
  soybean_weather2002,
  append(soybean_direct_modules, 'BioCro:nonexistent_module'), # add a nonexistent module
  soybean_differential_modules,
  soybean_ode_solver
)
@
...
requires a module specification and a named list of input quantities. It
...
<<c3_assimilation_v1>>=
outputs <- evaluate_module('BioCro:c3_assimilation', soybean_parameters)
@
...
This function requires a module specification, a list of fixed input quantities,
and a data frame specifying the values of any input quantities that should vary.
It returns a data frame with each of the module's inputs and outputs as columns,
where each row represents a different set of inputs. This is a convenient format
for plotting or saving the results, as discussed in Section
...
caption <- paste0(
  ...
  unique(rc$module_specification), '` module'
)
...
\texttt{BioCro}, so all module specifications currently take the form
...
\texttt{module\_specification}. Each row in this data frame represents an input
or output of one of the modules from the specified module library.
...
leaf_modules <- unique(leaf_quantity_subset$module_specification)
