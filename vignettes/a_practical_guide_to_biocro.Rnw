%\VignetteIndexEntry{A Practical Introduction to BioCro}
...
\hypersetup{
	pdftitle={A Practical Introduction to BioCro},
...
\title{A Practical Introduction to BioCro}
...
state $\boldsymbol{X}$ can be defined as the combination of the parameters,
drivers, differential quantities, and \emph{direct quantities} (the quantities
produced as outputs of direct modules). So, within the context of a valid
simulation, all the inputs ($\boldsymbol{X}_M^i$) and outputs
...
start of the simulation (where $t = t_0$).
