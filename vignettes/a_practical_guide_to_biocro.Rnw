...
\texttt{soybean\_parameters}, \texttt{soil\_parameters\$clay\_loam}, and
\texttt{soybean\_initial\_values}. Two separate parameter lists
(\texttt{soybean\_parameters} and \texttt{soil\_parameters\$clay\_loam}) were
combined with the \texttt{c} command to form one list with soybean \emph{and}
soil parameter values. As a whole, these lists include quantities that
characterize soybean physiological processes (such as $v_{\rm{c,max}}$),
specifics related to field conditions (such as the initial seed mass per unit
ground area), and other important categories of quantities. To learn about
specific quantities, see Section \ref{sec:quantity_info}.
...
soybean_result <- within(soybean, {run_biocro(
  within(initial_values, rm(Leaf)), # remove the initial `Leaf` value
  parameters,                       # don't include soil parameters
  soybean_weather$'2002',
  direct_modules,
  differential_modules,
  ode_solver
...
soybean_result <- within(soybean, {run_biocro(
  ...
  c(parameters, soil_parameters$clay_loam),
  soybean_weather$'2002',
  append(direct_modules, 'BioCro:nonexistent_module'), # add a nonexistent module
  differential_modules,
  ode_solver
)})
@
...
valid <- within(soybean, {validate_dynamical_system_inputs(
  initial_values,
  c(parameters, soil_parameters$clay_loam),
  soybean_weather$'2002',
  rev(direct_modules), # Reverse the order of the direct modules
  differential_modules
)})
@
