...
# Scaling Leaf Level Photosynthesis to the Canopy Level
...
variables: $A_{n,leaf} = A_{n,leaf}(Q_{in}, I_{abs}, C_a, T_a, h_a)$.
...
where $d \ell$ is the LAI of an infinitesimally small piece of leaf and
$A_{n,leaf}(\ell)$ is the net CO$_2$ assimilation rate per leaf area of that
small piece, located at cLAI $\ell$. In light of the comments above about this
notation, the inputs to the photosynthesis model used to calculate $A_{n,leaf}$,
such as $Q_{in}$, should also be evaluated at $\ell$.

This is an oversimplified way to represent $A_{n,canopy}$ because it ignores
several other key variables that influence $A_{n,leaf}$ besides $\ell$. A common
refinement of this model is to separately consider sunlit and shaded leaves. A
major difference between these two leaf classes is the dependence of $Q_{in}$ on
$\ell$. Sunlit leaves at any depth within the canopy all experience a very
similar value of $Q_{in}$, while shaded leaves generally receive less light
deeper within the canopy. When sunlit and shaded leaves are treated separately,
Equation \@ref(eq:AnCanopy) can be modified to
...
where $f_{sunlit}(\ell)$ and $f_{shaded}(\ell)$ are the fractions of sunlit and
shaded leaves at cLAI $\ell$. All leaves must be either sunlit or shaded, so
$f_{sunlit}(\ell) + f_{shaded}(\ell) = 1$. $A_{n,leaf}^{sunlit}$ and
$A_{n,leaf}^{shaded}$ are the leaf-level net CO$_2$ assimilation rates of
infinitesimally small areas of sunlit and shaded leaves.

# Multilayer Canopy Models

While Equation \@ref(eq:AnCanopyRefined) is useful for defining $A_{n,canopy}$,
it has limited practical value since it does not have an analytical solution for
most models of leaf-level photosynthesis. The simplest approach to numerical
integration is to approximate the integral in Equation \@ref(eq:AnCanopyRefined)
using a [Riemann sum](https://en.wikipedia.org/wiki/Riemann_sum) with the
interval $(0,L)$ divided into $N$ subintervals of equal size. In this case,
Equation \@ref(eq:AnCanopyRefined) becomes
...
As discussed above, this notation implies that the inputs to the leaf-level
photosynthesis model also change with $\ell$. For example,
$A_{n,leaf}^{sunlit}(\ell_i)$ is a shorthand for
$A_{n,leaf}(Q_{in}^{sunlit}(\ell_i), I_{abs}^{sunlit}(\ell_i),
C_a^{sunlit}(\ell_i), T_a^{sunlit}(\ell_i), h_a^{sunlit}(\ell_i))$, where we
have allowed for the possibility that all inputs may be different for sunlit and
shaded leaves. It is also important to note that $A_{n,leaf}^{sunlit}(\ell)$ and
$A_{n,leaf}^{shaded}(\ell)$ retain their original definitions after
approximating the integral by a sum; they are still the leaf-level net CO$_2$
assimilation rates of infinitesimally small areas of sunlit and shaded leaves at
$\ell$.

From a biological perspective, this approach to numerical integration can be
understood as dividing the crop canopy into $N$ layers of equal amounts of leaf
area. Thus, Equation \@ref(eq:AnCanopyMultilayer) is usually referred to as a
"multilayer canopy model." In this model, we assume that $A_{n,leaf}^{sunlit}$
for all sunlit leaves in a layer can be reasonably approximated by
$A_{n,leaf}^{sunlit}$ at the midpoint of that layer, and likewise for shaded
leaves.
...
becomes larger, the width of each layer becomes smaller, and so environmental
conditions across each layer become more uniform. Of course, if $N \rightarrow
\infty$, the Riemann sum in Equation \@ref(eq:AnCanopyMultilayer) converges back
to the integral in Equation \@ref(eq:AnCanopyRefined).

## Multilayer Canopy Model Examples
...
\begin{itemize}
    \item The beam flux just above the canopy $Q_{ob}$
    \item The diffuse flux just above the canopy $Q_{od}$
    \item The diffuse flux within the canopy $Q_d$
    \item The down-scattered flux within the canopy $Q_{sc}$
    \item The total beam flux (unintercepted beam and downscattered beam) within
          the canopy $Q_{bt}$
    \item The unintercepted beam flux within the canopy $Q_b$
\end{itemize}
...
\begin{equation}
    Q_d(\ell) = Q_{od} \cdot e^{- \sqrt{\alpha_{diffuse}} \cdot K_{diffuse} \cdot \ell},
\end{equation}

where $\alpha_{diffuse}$ is the diffuse-light absorptivity of the leaves in the
canopy and $K_{diffuse}$ is the extinction coefficient for diffuse radiation
(see Figure 15.4 and surrounding discussion). The factor $\sqrt{\alpha}$ models
the effect of light scattering and transmission by the leaves. This is Equation
15.17. In BioCro, we use $\alpha_{diffuse} = 1$.
...
\begin{equation}
    Q_{bt}(\ell) = Q_{ob} \cdot e^{- \sqrt{\alpha_{direct}} \cdot K_{direct} \cdot \ell}
\end{equation}
...
Here we use the absorptivity for direct radiation $\alpha_{direct}$. These are
Equations 15.20, 15.15, and 15.16.
...
The total incident light energy can be estimated from the incident PPFD by
assuming a value for the average energy per PAR photon and the fraction of light
in the PAR band:

\begin{equation}
    I_{in} = \frac{Q_{in}}{E \cdot f_{PAR}},
\end{equation}

where $E$ is the average energy per PAR photon and $f_{PAR}$ is the fraction of
light energy in the PAR band. Then the absorbed shortwave energy can be found
using the equation for absorption by a thin layer, which simply multiplies the
incident radiation by the absorptivity:

\begin{equation}
    I_{abs} = \alpha \cdot \frac{Q_{in}}{E \cdot f_{PAR}}.
\end{equation}

Note that since the incident PPFD $Q_{in}$ is different for sunlit and shaded
leaves, $I_{abs}$ is similarly different, and we can distinguish between
$I_{abs}^{sunlit}$ and $I_{abs}^{shaded}$.

In reality, PAR and NIR are scattered and absorbed at different rates by leaves
in the canopy, so $f_{PAR}$ changes throughout the canopy. Currently BioCro
assumes a constant value for $f_{PAR}$. A better approach would be to separately
treat PAR and NIR scattering, determining each of the absorbed PAR and NIR
energy as functions of $\ell$ rather than using $f_{PAR}$.
...
Now we can plot incident light levels for leaves within the canopy. Here
...
aq_curve <- module_response_curve(
    ...
    within(
        expand.grid(
            ...
            Qabs = seq(0, total_ppfd * soybean$parameters$absorptivity_par, by = 5)
        ),
        {temp = Tleaf}
    )
)
...
# Include incident Q
aq_curve$Qin <- aq_curve$Qabs / soybean$parameters$absorptivity_par
...
```{r}
Q_abs_canopy <-
    soybean$parameters$absorptivity_par * (LAI / 10) *
