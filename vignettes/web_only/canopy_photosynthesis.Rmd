...
# Calculate PPFD and NIR just above the canopy
shortwave_res <- evaluate_module(
    'BioCro:incident_shortwave_from_ground_par',
    within(soybean$parameters, {
        irradiance_diffuse_fraction = 1 - beam_fraction
        irradiance_direct_fraction = beam_fraction
        solar = total_ppfd
    })
)
...
canopy_properties <- evaluate_module(
    ...
    within(soybean$parameters, {
        ...
        lai = LAI
        nir_incident_diffuse = shortwave_res$nir_incident_diffuse
        nir_incident_direct = shortwave_res$nir_incident_direct
        ppfd_incident_diffuse = shortwave_res$ppfd_incident_diffuse
        ppfd_incident_direct = shortwave_res$ppfd_incident_direct
        windspeed = windspeed
    })
)
