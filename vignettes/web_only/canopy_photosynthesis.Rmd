...
variables: $A_n = A_n(Q_{in}, I_{abs}, C_a, T_a, h_a)$.
...
With this in mind, $A_{n,leaf}$ can also be expressed as function of $\ell$
rather than each individual variable. This notation is convenient because it
allows us to integrate $A_{n,leaf}$ across the entire canopy to define the net
CO$_2$ assimilation rate per unit ground area ($A_{n,canopy}$) as
...
small piece, located at cLAI $\ell$. Thus, the inputs to the photosynthesis
model used to calculate $A_{n,leaf}$, such as $Q_{in}$, should also be evaluated
at $\ell$.
...
The multilayer canopy model expressed in Equation \@ref(eq:AnCanopyMultilayer)
relies on an assumption that the environmental conditions of sunlit (or shaded)
leaves in each layer are fairly constant across the layer. For example, once
$A_{n,leaf}^{sunlit}(\ell_i)$ and $f_{sunlit}(\ell_i)$ have been calculated, the
total contribution of sunlit leaves in layer $i$ is found by multiplying the
leaf-level rate ($A_{n,leaf}^{sunlit}(\ell_i)$) by the total sunlit leaf area in
the layer ($f_{sunlit}(\ell_i) \cdot L / N$). This only makes sense if $L / N$
is relatively small, so that incident light or other variables do not change
much across the layer.
...
and shaded leaves ($Q_{sunlit}$ and $Q_{shaded}$) are

\begin{equation}
    Q_{sunlit} = K_{direct} \cdot Q_{ob} + Q_d + Q_{sc}
\end{equation}
...
(Equations 15.18 and 15.19). The term $K \cdot Q_{ob}$ is the average direct
beam radiation accounting for the distribution of leaf angles.
...
leaves, $I_{abs}$ is similarly different.
