...
\begin{itemize}
  \item This workshop was originally held on May 13, 2022 as part of the
  \href{https://cropsinsilico.org/2022-annual-meeting/}{Crops in Silico Symposium}.

  \item Since then, it has been updated following changes to BioCro syntax and
  other new developments.

  \item To use the workshop on your own, try running the code snippets on your
  machine, and complete the exercises to test your understanding.
\end{itemize}

\noindent Throughout this document, look out for the following types of boxes:
...
\begin{itemize}
  ...
  \item We have published a paper describing it in \textit{in silico Plants}
  ...
  \item All BioCro documentation can be accessed at \url{https://biocro.org}.
\end{itemize}
...
\begin{itemize}
  ...
  \item A more detailed description of BioCro's approach to modular modeling can
  be found in Section \ref{sec_modular}.
\end{itemize}
...
\begin{itemize}
  ...
  \item The primary developers in recent years include Justin McGrath, Ed
  Lochocki, Yufeng He, Scott Oswald, Scott Rohde, Deepak Jaiswal, Megan
  Matthews, Fernando Miguez, and Steve Long.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{Parameterization}: Find parameter values that best reproduce
...
\noindent If you see a warning about the version of BioCro, or if any of the
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    to calculate the CO$_2$ assimilation rate, stomatal conductance to H$_2$O,
...
\noindent Note that the \texttt{?} command can be applied to all functions
...
\begin{itemize}
  \item Analogous to evaluating an expression of code, we can evaluate a
  module's equations given a set of inputs.
...
\begin{itemize}
  \item In the previous example, a single value was provided for each of the
  module inputs, so each output also had a single value.
...
\noindent The basic Miscanthus model is included with the BioCro R package, as
...
\noindent Next, we'll create a function that runs a miscanthus growth simulation
...
\noindent Now we can check the result. In these plots, simulated values are
...
\noindent This optimization may seem complicated, but it is actually much
...
\noindent Then, the final pod biomass can be extracted.
...
\noindent For example, you could try increasing a photosynthesis parameter like
...
\noindent In this simulation, photosynthesis was included via the mechanistic
...
\noindent This will reveal some differences between the response of C$_3$ and
...
\noindent See \cite{lochocki_biocro_2022} for a more detailed example of
