...
package (which provides tools for creating plots), the \texttt{nloptr}
package (which provides tools for optimization), and the \texttt{deSolve}
...
system.time({
  nlresult = nloptr(
    x0=unlist(sorghum$parameters$iSp),
    eval_f=to_minimize,
    opts=opts,
    lb=0.001,
    ub=100
...
setNames(nlresult$solution, 'iSp')
@
\end{boxCode}
