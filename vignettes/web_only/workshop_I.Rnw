...
parameters whose values must be determined. A full example demonstrating one
approach to parameterizing BioCro's soybean model can be found online at the
documentation web site for the \texttt{BioCroValidation} R package:
...
<<simple_soybean_optimization, eval = FALSE>>=
...
Try varying a different parameter instead of \texttt{alphaLeaf}.
