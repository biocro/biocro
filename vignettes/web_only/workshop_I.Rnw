...
<<eval=FALSE>>=
# Make sure packages are installed. `BioCro`, `lattice` and `nloptr` are
# available from CRAN, so they can be installed from the remote package
# repository.
...
Then, they can be loaded into the R workspace as follows:
...
Section \ref{sec_additional_examples}, try something on your own, or ask any
...
the soybean model as \texttt{Vcmax\_at\_25}:
...
\section{Understanding BioCro's approach to modular modeling}
