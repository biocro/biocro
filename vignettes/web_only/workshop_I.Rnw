...
package (which provides tools for creating plots), and the \texttt{deSolve}
package (which provides tools for solving differential equations). We will load
them now before moving on to the examples.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    initial specific leaf area (\texttt{iSp}).
...
stored in \texttt{.csv} files. These can be downloaded from the
\href{https://github.com/biocro/biocro/tree/main/vignettes/web_only}{BioCro GitHub page}.
Then, they can be loaded into the R workspace.
...
<<test_sorghum_model, include=FALSE>>=
# This is used for debugging purposes when updating the sorghum model and
# shouldn't be included in the actual vignette
sorghum_valid <- with(sorghum, {validate_dynamical_system_inputs(
  initial_values,
  parameters,
  climate,
  direct_modules,
...
if (!sorghum_valid) {
  stop('sorghum model definition is not valid')
}

sorghum_test_res <- with(sorghum, {run_biocro(
  initial_values,
  parameters,
  climate,
  direct_modules,
...
using the weather data but accepts only \texttt{iSp} as an input; this can be
accomplished using \texttt{partial\_run\_biocro}. When comparing to the measured
data, we'll need the above-ground dry yield; this isn't included in the typical
simulation output, but can be calculated afterwards.
...
system.time({
  sorghum_optim_result <- optim(
    unlist(sorghum$parameters$iSp),
    to_minimize,
    method = 'Brent',
    lower = 0.001,
    upper = 10
...
setNames(sorghum_optim_result$par, 'iSp')
@
\end{boxCode}
...
\begin{boxCode}
<<sorghum_optimizer_check, eval = FALSE>>=
r = partial_function(sorghum_optim_result$par)
