...
add_photoperiod_length <- function(weather_data) {
    clock_output <- with(soybean_clock, {
        run_biocro(
            initial_values,
            parameters,
            weather_data,
...
fvcb_result_2002 <- with(cmi_soybean, {
    run_biocro(
        initial_values,
        parameters,
        soy_weather[["2002"]],
...
optimal_rue_result_2002 <- with(cmi_soybean, {
    run_biocro(
        initial_values,
        within(parameters, {
            alpha_rue <- best_alpha_rue
        }),
        soy_weather[["2002"]],
        within(direct_modules, {
            canopy_photosynthesis <- "BioCro:ten_layer_rue_canopy"
...
biomass_comparison_2002 <- rbind(
    within(fvcb_result_2002[biomass_columns], {
        model <- "FvCB"
    }),
    within(optimal_rue_result_2002[biomass_columns], {
        model <- "RUE"
    })
)
    ...
    fvcb_sensitivity_light_curve_result[[varname]] <-
        apply(
            ...
            function(x) {
                assim_sensitivity(
                    varname,
                    c(within(cmi_soybean$parameters, {
                        rm(Catm)
                    }), as.list(x))
                )
            }
...
biomass_driver_sensitivity <- function(
    ...
    default_drivers <- within(soy_weather[["2002"]], {
        temp <- temp + c_to_k
    })

    default_result <- with(cmi_soybean, {
        run_biocro(
            initial_values,
            sens_parameters,
            within(default_drivers, {
                temp <- temp - c_to_k
            }),
            within(direct_modules, {
...
biomass_temp_sensitivity <- rbind(
    within(biomass_temp_sensitivity_fvcb, {
        model <- "FvCB"
    }),
    within(biomass_temp_sensitivity_rue, {
        model <- "RUE"
    })
)
...
biomass_catm_sensitivity <- rbind(
    within(biomass_catm_sensitivity_fvcb, {
        model <- "FvCB"
    }),
    within(biomass_catm_sensitivity_rue, {
        model <- "RUE"
    })
)
...
fvcb_result_2006 <- with(cmi_soybean, {
    run_biocro(
        initial_values,
        within(parameters, {
            Catm <- catm[["2006"]]
...
rue_result_2006 <- with(cmi_soybean, {
    run_biocro(
        initial_values,
        within(parameters, {
            alpha_rue <- best_alpha_rue
...
biomass_comparison_2006 <- rbind(
    within(fvcb_result_2006[biomass_columns], {
        model <- "FvCB"
    }),
    within(rue_result_2006[biomass_columns], {
        model <- "RUE"
    })
)
