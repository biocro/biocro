...
assimilation rate:
\begin{equation}
A_n = V_c - 0.5 \cdot V_o - R_L, (\#eq:an)
\end{equation}
where $V_c$ is the RuBP carboxylation rate, $V_o$ is the RuBP oxygenation rate,
and $R_L$ is the rate of non-photorespiratory CO2 release in the light. As
written, $V_c$, $V_o$, and $R_L$ are all non-negative rates, typically with
units of $\mu$mol m$^{-2}$ s$^{-1}$. The first two terms in the equation
($V_c - 0.5 \cdot V_o$) represent the contribution of photosynthesis (or
Rubisco) to the net CO~2~ assimilation, while the remaining term ($-R_L$) is the
result of non-photorespiratory CO2 release, a separate process that includes
mitochondrial respiration as well as the pentose phosphate pathway. The rate of
non-photorespiratory CO2 release is known to depend on the amount of incident
light, and is usually specified either in the light (as we do here) or in the
dark.
...
\begin{equation}
A_n = V_c \cdot \left( 1 - 0.5 \cdot \phi \right) - R_L. (\#eq:anphi)
\end{equation}
...
\begin{equation}
A_n = V_c \cdot (1 - \Gamma^* / C) - R_L (\#eq:anfinal)
\end{equation}
...
\begin{equation}
A_c = W_c \cdot (1 - \Gamma^* / C) - R_L = \frac{V_{cmax} \cdot \left( C - \Gamma^* \right)}{C + K_c \cdot \left( 1 + O / K_o \right)} - R_L. (\#eq:ac)
\end{equation}
...
\begin{equation}
A_j = W_j \cdot (1 - \Gamma^* / C) - R_L = \frac{J \cdot \left( C - \Gamma^* \right)}{4 \cdot C + 8 \cdot \Gamma^*} - R_L. (\#eq:aj)
\end{equation}
...
$A_c = W_c \cdot \left( 1 - \Gamma^* / C \right) - R_L$ is also negative.
...
\begin{equation}
A_p = W_p \cdot (1 - \Gamma^* / C) - R_L = \frac{3 \cdot T_p \cdot \left( C - \Gamma^* \right)}{C - \Gamma^* \cdot \left( 1 + 3 \cdot \alpha \right)} - R_L. (\#eq:ap)
\end{equation}
...
$C = 0$:
\begin{equation}
A_c \big|_{C = 0} = - \frac{\Gamma^* \cdot V_{cmax}}{K_c \cdot \left( 1 + O / K_o\right)} - R_L
\end{equation}
and
\begin{equation}
A_j \big|_{C = 0} = - \frac{J}{8} - R_L.
\end{equation}
...
pseudo_fvcb_model <- function(
    ...
    Ac <- Vcmax * (C - Gstar) / (C + Kc * (1 + O / Ko)) - RL

    Aj <- J * (C - Gstar) / (4 * C + 8 * Gstar) - RL

    Ap <- 3 * Tp * (C - Gstar) / (C - Gstar * (1 + 3 * alpha)) - RL
...
```{r, echo = FALSE}
do.call(fvcb_compare, within(defaults, {
    plot_type <- 1
}))
```
