...
validated soybean growth model using weather data corresponding to the 2002
growing season in Champaign, Illinois. In this section, we discuss the R
functions and input arguments required to accomplish this goal. Along the way,
we will introduce the most important R data types for BioCro analysis:
...
during the specified time period. The BioCro package includes pre-set
collections of input arguments that can be passed to \texttt{run\_biocro} when
...
relate to the central element of BioCro philosophy: that \emph{models are sets
of equations}. Information about \texttt{run\_biocro} can also be obtained from
R's built-in help system using the following command:
...
In this case, it is helpful to define a collection of modules in R using a
\emph{list} rather than a \emph{vector}. With a list, it is possible to add a
name for some or all of the list elements. For example, we could write the
following:
...
?c
?list
?`$`    # we have to wrap this operator in backticks when using ?
?`<-`   # we have to wrap this operator in backticks when using ?
?`=`    # we have to wrap this operator in backticks when using ?
?print
?str
...
-- that is, quantities whose values are taken to be constant during the course
of a simulation -- as well as the \emph{initial values} of any quantities
whose evolution with time is determined by differential equations. In general,
the sets of required parameters and initial values will be determined by the
modules and drivers (see Section \ref{sec:choose_drivers}) that a user has
chosen to use.

Both the parameters and initial values must be specified in R using lists of
named elements. When specifying collections of modules (Section
\ref{sec:choose_modules}), names are optional; for parameters and initial
values, names are required since they are used to uniquely ideniify each
quantity. For example, if we have two parameters \texttt{parameter\_1} and
\texttt{parameter\_2} whose values are 2.3 and 8.9, respectively, we can specify
them using a list as follows:
...
Here the syntax of defining a list -- where a value is assined to a name using
an equals sign -- is a reminder that parameter specifications can be thought of
as individual equations of the form $x_i(t) = p_i$, where $x_i$ is a variable,
$x_i(t)$ is its time dependence, and $p_i$ is a constant value.
...
temperature (\texttt{temp}) throughout one day at 3-hour time steps as follows:

<<example_drivers>>=
hour <- seq(0, 23, 3)
temp <- 20 + 8 * sin((hour / 24) * pi) # we use "vector arithmetic" to form `temp`
drivers <- data.frame(
    hour = hour,
    temp = temp
)
...
The contents of a data frame can be viewed in several ways; here we print to the
R terminal using the \texttt{print} command:
...
represents a time series of quantity values. Within the BioCro framework, linear
...
equation of the form $x_i(t) = d(t)$, where $x_i$ is a variable, $x_i(t)$ is its
time dependence, and $d(t)$ is a continuous function determined from the
discrete values of the driver defined in the data frame.
...
options can be obtained using the \texttt{get\_all\_ode\_solvers()} function.

\item \texttt{output\_step\_size}: The time step to be used in the output of
...
was introduced in Section \ref{sec:choose_drivers}, which discusses drivers. In
this case, each column represents one of the quantities whose value is
determined during the simulation, and each row represents a time point. By
default, all quantities except the parameters are included in the output of
\texttt{run\_biocro}, so this data frame also includes the drivers.
...
times. For example, we could view just the first time point, all times between
the 10th and 20th points, or all hours of day 250:

<<print_subset_narrow>>=
print(soybean_result[1, c('doy', 'hour', 'Leaf')])
print(soybean_result[10:20, c('doy', 'hour', 'Leaf')])
print(soybean_result[soybean_result$doy == 250, c('doy', 'hour', 'Leaf')])
@
...
In that example, we specified the \texttt{y} values by extracting the
...
by typing \texttt{?xyplot} or by consulting one of the many guides available
online that discuss creating plots in R with the \texttt{lattice} package.
...
Basic module information -- its input quantities, output quantities, and type
-- can be obtained from within R using the \texttt{module\_info} function,
...
It should be mentioned that the Doxygen documentation also includes the code
itself in addition to the nicely-formatted comments.
...
To understand how a module works, it can be helpful to visualize of how one of
its output quantities depends on one of its input quantities across a
reasonable range; a plot typically called a \emph{response curve}. As an
example, here we will calculate the response of the soybean net CO$_2$
assimilation rate ($A_n$) to the incident photosynthetically active photon flux
density (PPFD; $Q$) according to the Farquhar-von-Caemmerer-Berry model for
C$_3$ photosynthesis, which is available in BioCro as the
...
defined. We can choose a values for these and add them to the parameter list:
...
using the strategy in Section \ref{sec:quantity_info}.
...
discussed previously in Sections \ref{sec:run_simulation} and
\ref{sec:choose_modules}.

A response curve can also be the first step towards a full sensitivity
analysis, where numerical derivatives of an output quantity with respect to an
input quantity can be calculated. See the \emph{Quantitative Comparison Between
Two Photosynthesis Models} vignette for a more detailed analysis including some
of these ideas.
