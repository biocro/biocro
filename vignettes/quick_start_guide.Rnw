...
of equations}. Information about \texttt{rn\_biocro} can also be obtained from
...
Most users will only be interested in the source code for the modules, which is
...
take in CO$_2$ during the day, they exhale CO$_2$ at night. As light increases,
the initial response is linear, although it begins to plateau at higher light
intensities; in other words, the additional light energy does not cause a large
increase in assimilation when the light intensity is already high.
...
Thus, the ability to quickly and easily calculate response curves from
individual model components is another key ability of BioCro, which complements
the benefits of modularity that were discussed previously in Sections
\ref{sec:run_simulation} and \ref{sec:choose_modules}.
