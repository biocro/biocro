...
for one or more members $x_i$ of the model's state $\boldsymbol{X}$, which is
taken to comprise all quantities involved in the model (Equations
...
of plant biology. Each module $M$ has a set of \emph{input quantities}
$\boldsymbol{X}_M^i$ and a set of \emph{output quantities} $\boldsymbol{X}_M^o$;
when a module is run, its outputs are determined from the values of its inputs
according to its equations. Modules come in two types:
\emph{differential modules} calculate terms of the time derivatives of their
output quantities, while \emph{direct modules} directly calculate the
instantaneous values of their output quantities. In other words, a direct module
defines an equation of the form
\begin{equation}
\boldsymbol{X}_M^o = f_M \left( \boldsymbol{X}_M^i , t \right)
\end{equation}
and a differential module defines an equation of the form
\begin{equation}
\frac{d \boldsymbol{X}_M^o}{dt} = g_M \left( \boldsymbol{X}_M^i , t \right).
\end{equation}

Within the context of a simulation, certain rules apply to the inputs and
outputs of modules to ensure that the system is valid (in other words, that it
is well-defined and can in principle be solved). The rules are as follows:
\begin{enumerate}

\item Since direct modules calculate instantaneous values of quantities, at most
one direct module can have a particular quantity $x$ as an output. For example,
multiple direct modules available in the library calculate the canopy
assimilation rate, but only one such module can be used in a single simulation.

\item Since differential modules calculate terms of a derivative, multiple
differential modules can have a particular quantity $x$ as an output; in this
case, the module outputs will be added together to determine the overall
derivative. For example, one differential module may calculate a rate of leaf
carbon gain (due to assimilation) while another calculates a negative rate of
leaf carbon loss (due to senescence); both may be used in a single simulation.

\item Since the output quantities from differential modules (the
\emph{differential quantities}) evolve according to differential equations, they
also require initial values. One initial value is required for each differential
quantity (Section \ref{sec:choose_param}).

\item Within a simulation, the direct modules are run in a specified order,
allowing the formation of chains where the output of one direct module is used
as the input to another direct module. Thus, a requirement on the system as a
whole is that the inputs for each direct module must be \emph{defined} before it
runs; here, an input quantity is said to be defined if its value is known at the
start of the simulation---in other words, if it is a parameter
(Section \ref{sec:choose_param}), an initial value
(Section \ref{sec:choose_param}), or a driver
(Section \ref{sec:choose_drivers})---or if it is the output of a direct module
that was previously run.

\item Cyclical dependencies among the direct modules are not permitted; for
example, if module $A$ takes quantity $b$ as an input and produces quantity $a$
as an output ($b \rightarrow A \rightarrow a$) while module $B$ takes quantity
$a$ as an input and produces quantity $b$ as an output ($a \rightarrow B
\rightarrow b$), this would be a cyclical dependency.

\item Within a simulation, the differential modules are run after the direct
modules, allowing the formation of chains where the output of a direct module is
used as the input to a differential module. For example, one direct module may
calculate partitioning coefficients based on a plant's physiological age, and a
differential module can subseqently use those parititioning coefficients to
distribute assimilated carbon to different organs. Thus, a requirement on the
system as a whole is that the inputs for each differential module must be
defined before it runs.

\item There must be a possible ordering for the modules that ensures that each
module input is defined before it runs.

\end{enumerate}

If all these conditions are satisfied (Section \ref{sec:validate}), then the
elements of the state $\boldsymbol{X}$ can be defined as the combination of the
parameters, drivers, differential quantities, and \emph{direct quantities} (the
quantities produced as outputs of direct modules). So, within the context of a
valid simulation, all module inputs ($\boldsymbol{X}_M^i$) and outputs
($\boldsymbol{X}_M^o$) of each module $M$ are subsets of the state
$\boldsymbol{X}$. Additionally, a direct module cannot have any overlap between
its inputs and outputs. With this in mind, we sometimes gloss over the detailed
requirements for the module inputs and outputs and say that collection of direct
modules defines a set equations of the form
\begin{equation}
x_i = \boldsymbol{f}_i \left( \boldsymbol{X}^i, t \right),
\label{eq:direct}
\end{equation}
where $x_i$ is an element of the state $\boldsymbol{X}$ and $\boldsymbol{X}^i$
represents the other state quantities besides $x_i$; likewise, we say that the
collection of differential modules defines a set of equations of the form
\begin{equation}
\frac{dx_i}{dt} = \boldsymbol{g}_i \left( \boldsymbol{X}, t \right).
...
\end{equation}
where $x_i$ is an element of the state $\boldsymbol{x}$, $p_i$ is a constant
parameter value and $x_i(t_0)$ is the value of a differential quantity at the
start of the simulation (were $t = t_0$).
...
representing simple individual equations like Equations \ref{eq:param} and
...
\label{eq:driver}
\end{equation}
where where $x_i$ is a member of the state $\boldsymbol{X}$ and $d_i(t)$ is a
function determined by interpolation from the driver's discrete values.
...
\texttt{soybean\_weather2002}. Assembling drivers that represent weather data in
a particular location can be a complicated process because different weather
data sets or weather sensors produce different outputs and use different
...
algorithm as an \emph{ode\_solver}.
...
supplied arguments are able to form a valid dynamical system (Section
\ref{sec:choose_modules}) and provides helpful feedback to the user if a problem
is detected. For example, if one or more of the differential quantities is
missing an initial value, or if a parameter is missing, an error will occur:
...
simulation. It provides additional information, such as a list of parameters
that are not used by any modules, since such parameters could in principle be
removed without affecting the simulation.
...
requires a module name and a named list of input quantities. It attempts to run
the module, returning its outputs as a named list. If any of the module's inputs
are not provided, a helpful error message will be returned. So we can try
passing the \texttt{soybean\_parameters} list as an input to the
...
other inputs and returns a value for $A_n$. In this function, we pass the value
of $Q$ to the module via its \texttt{Qp} input, and then extract the module's
\texttt{Assim} output from the list returned by \texttt{evaluate\_module}:
...
which prints module info to the R terminal and also returns it as a list.
Printing to the terminal can be disabled by setting the \texttt{verbose}
argument to \texttt{FALSE}. For example:
...
Notice that \texttt{info\$inputs} and \texttt{info\$outputs} are lists of named
numeric elements where each element has been initialized to 1; the input list
can be particularly useful since it is already in the correct format to pass to
the module via \texttt{evaluate\_module}, typically following a modification of
one or more of its values:

<<module_evaluation>>=
str(evaluate_module('total_biomass', within(info$inputs, {Leaf = 2})))
@

Also notice that BioCro modules can be designated as requiring a fixed-step
Euler solver; this is to accommodate discrete dynamical systems, which can be
...
non-Euler ODE solver when one or more of its modules requires an ODE Euler
solver.

A list of all available modules can be obtained with the
\texttt{get\_all\_modules} function, and  detailed information about an
individual module can be obtained by viewing its source code or the associated
Doxygen documentation; see Section \ref{sec:source_code}.
...
\section{Additional R commands and resources \label{sec:extra_help}}
...
\url{https://www.statmethods.net/management/subset.html}.
...
\url{http://www.r-tutor.com/r-introduction/vector/vector-arithmetics}.
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.7\textwidth]{quick_start_guide/soybean_vcmax_exercise.pdf}
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.7\textwidth]{quick_start_guide/temperature_response_curve.pdf}
    \caption{Temperature response of soybean net assimilation.}
