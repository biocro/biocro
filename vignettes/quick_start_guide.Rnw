...
of strings. For example, we could write:
...
equals sign (\texttt{=}) or an arrow (\texttt{<-}). The assignment operator
(arrow) is used much more often, so we will use it here as well.
...
Besides the modules, the user must also provide:
\begin{itemize}
...
For this example, we will use weather data from Champaign, IL during 2002 along
with the default soybean parameters, initial values, and ODE solver; all of
these are also provided as lists that are part of the BioCro package.
...
other hand, if the name of an element is stored as a variable, it cannot be
used.
...
need to define a single equation. To learn more about the \texttt{run\_biocro}
function, see the file \texttt{run\_biocro.R} in the BioCro repository, which
...
simulation. It also provides additional information such as a list of parameters
...
(mega grams per hectare). You may be wondering: where did this information come
from?
...
outputs <- evaluate_module(
  ...
  within(soybean_parameters, {
    ...
    vmax = vmax1  # we need to rename this parameter
    bb0 = b0      # we need to rename this parameter
    bb1 = b1      # we need to rename this parameter
  })
)
@
...
outputs of a BioCro simulation, published literature, colleagues, etc.
Critica information about particular quantities, such as their meaning and
units, can be obtained using the strategy in Section \ref{sec:quantities}.
