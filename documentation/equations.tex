...
%\usepackage{pdfpages}
%\usepackage{caption}
\usepackage[backgroundcolor=white, textsize = small, ]{todonotes}
\usepackage{marginnote}
\usepackage[top=2cm, bottom=1.3cm, left=0.5cm, right=5cm, heightrounded,
...
\end{align}
\todo[inline]{what is the relevance of equation \ref{eqn:costlen}? Steve H's thesis contains the original, below}
\todo[inline]{$\lambda$ is defined as ``specific heat of vaporization''; not sure what it is here. The orignial eqn. used $\Omega$}
...
\end{align}
\todo[inline]{what are $k_t$ and $K_t$ in equations \ref{eqn:Vcmax}}
...
\end{align}
\todo[inline]{renamed $\beta$ as to $\beta_\Phi$; is this an appropriate naming?}
...
\end{align}
\todo[inline]{is $\ell_\text{sun}\equiv_\text{sun}$? neither is defined}
\todo[inline]{why use $\int$ instead of $\sum$?}
